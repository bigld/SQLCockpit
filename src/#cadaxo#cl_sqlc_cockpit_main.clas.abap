CLASS /cadaxo/cl_sqlc_cockpit_main DEFINITION
  ...
  PUBLIC SECTION.
    ...
    METHODS get_sql_area
      RETURNING
        VALUE(e_code_string) TYPE string
      RAISING
        /cadaxo/cx_sqlc_syntax_error .
    "! get sql area
    ...
    DATA symbols_controller TYPE REF TO /cadaxo/cl_sqlc_symbols .
  PROTECTED SECTION.
  ...
  PRIVATE SECTION.
    ...
    DATA g_abap_editor_type TYPE char1 VALUE editor_type-new ##NO_TEXT.
    ...
    CLASS-DATA g_version_nr TYPE string VALUE '3.9' ##NO_TEXT.
...
*    FIELD-SYMBOLS: <l_cl_sql_parse>     TYPE REF TO /cadaxo/cl_sqlc_cockpit_parse.
    ...
    TRY.
        ...
        /cadaxo/cl_sqlc_cockpit_parse=>parse_sql_i( EXPORTING i_sql           = i_sql_string
...
* INSERT SERILIZATION END
  ...
  METHOD api_saved_list_import.
    ...
    ir_api->get_item( EXPORTING iv_pos_line = is_items
                      IMPORTING rt_item     = lt_saved_list ).
    ...
    IF update_ok = abap_true.
      ...
      set_result_toolbar_active( i_fcode = c_cmd_show_saved_lists ).
      show_saved_lists( ).
    ELSE.
      ROLLBACK WORK.
  ...
  METHOD calc_result_rows_and_cols.
    ...
    DATA l_calc TYPE p LENGTH 8 DECIMALS 2.
    ...
    CASE ms_user_settings_xml-reswindoworientation.
      ...
      WHEN OTHERS.
        ...
        CALL FUNCTION 'ROUND'
          EXPORTING input  = l_calc
                    sign   = 'X'
          IMPORTING output = e_rows.

        CALL FUNCTION 'ROUND'
          EXPORTING input  = l_calc
                    sign   = '+'
          IMPORTING output = e_cols.
    ENDCASE.
  ENDMETHOD.
  ...
  METHOD check_dbtable_modification.
    ...
    LOOP AT lt_tables ASSIGNING FIELD-SYMBOL(<table>).
      ...
      IF sy-subrc <> 0 OR ddic_header-crstamp IS INITIAL.
        CONTINUE.
      ENDIF.
      ...
      IF sy-subrc = 0 AND ( timestamp > g_trstart_timestamp ).

        MESSAGE s159(/cadaxo/sqlc) WITH <table> INTO DATA(message).

        RAISE EXCEPTION NEW /cadaxo/cx_sqlc_syntax_error(
  ...
  METHOD check_sql_syntax.
    ...
    FIELD-SYMBOLS <ls_scirestps>    TYPE scir_rest.
    ...
    TRY.
        ...
        IF     l_sql_string IS NOT INITIAL
          ...
          /cadaxo/cl_sqlc_cockpit_parse=>check_sql_syntax( EXPORTING i_sql_parsed = <lt_cl_sql_parse>
        ...
        ELSE.
          ...
          RAISE EXCEPTION NEW /cadaxo/cx_sqlc_syntax_error( message       = l_message
  ...
  METHOD class_constructor.
    ...
    /cadaxo/cl_sqlc_cockpit_assist=>get_parameter_value(
      EXPORTING  i_parameter_id    = /cadaxo/cl_sqlc_cockpit_assist=>c_param_version
    ...
    IF sy-subrc = 0.
      ...
      TRY.
        ...
        CATCH cx_parameter_invalid_range
              ...
              cx_sy_compression_error ##NO_HANDLER.
      ENDTRY.
      ...
      TRY.
          ...
          me->g_user_settings-use_convexit         = me->ms_user_settings_xml-use_convexit.        " Convers Routine
          me->g_user_settings-show_footer          = me->ms_user_settings_xml-show_footer.         " Show ALV Footer #4093
          me->g_user_settings-maxsel               = me->ms_user_settings_xml-maxsel.              " Max Rows
          me->g_user_settings-result_buttons       = me->ms_user_settings_xml-result_buttons.      " Show Result Buttons
          me->g_user_settings-save_clipboard       = me->ms_user_settings_xml-save_clipboard.      " Save Clipboard
          me->g_user_settings-result_doubleclick   = me->ms_user_settings_xml-result_doubleclick.  " Result Doubleclick
          me->g_user_settings-sql_trace            = me->ms_user_settings_xml-sql_trace.           " SQL Trace           "CDX001-0008
          me->g_user_settings-tablebuffer_trace    = me->ms_user_settings_xml-tablebuffer_trace.   " Tablebuffer Trace   "CDX001-0008
          me->g_user_settings-symbols_show         = me->ms_user_settings_xml-symbols_show.        " Symbols ALV "CDX001-0020
          me->g_user_settings-symbols_program_show = me->ms_user_settings_xml-symbols_program_show. " Programsymbols show "CDX001-0020
          me->g_user_settings-only_used_symbols    = me->ms_user_settings_xml-only_used_symbols.   " CR22-002
          me->g_user_settings-history_last_x_days  = me->ms_user_settings_xml-history_last_x_days.
          me->g_user_settings-job_last_x_days      = me->ms_user_settings_xml-job_last_x_days.
          me->g_user_settings-hd_show_alias        = me->ms_user_settings_xml-hd_show_alias.        " Show alias in header
          me->g_user_settings-hd_fieldtext_l       = me->ms_user_settings_xml-hd_fieldtext_s.
          me->g_user_settings-hd_fieldtext_m       = me->ms_user_settings_xml-hd_fieldtext_m.
          me->g_user_settings-hd_fieldtext_l       = me->ms_user_settings_xml-hd_fieldtext_l.
          me->g_user_settings-hd_fieldtext_a       = me->ms_user_settings_xml-hd_fieldtext_a.
          me->g_user_settings-editor_type          = me->ms_user_settings_xml-editor_type.
          me->g_user_settings-forwnavddleclipse    = me->ms_user_settings_xml-forwnavddleclipse.
          me->g_user_settings-forwnavdicteclipse   = me->ms_user_settings_xml-forwnavdicteclipse.
          me->g_user_settings-domaintext           = me->ms_user_settings_xml-domaintext.           " COCKPIT-458
          me->g_user_settings-release_type         = me->ms_user_settings_xml-release_type.         " COCKPIT-98
          me->g_user_settings-release_number       = me->ms_user_settings_xml-release_number.       " COCKPIT-98

          " Column Header - Fieldname or Fieldtext
          CASE me->ms_user_settings_xml-colhd_type.
            WHEN '1' OR space.
              me->g_user_settings-hd_fieldname = abap_true.
            WHEN '2'.
              me->g_user_settings-hd_fieldtext = abap_true.
          ENDCASE.
        ...
        CATCH cx_xslt_runtime_error.
          " fill default values
          me->g_user_settings-use_convexit = abap_true.          " Convers Routine
          me->g_user_settings-maxsel       = 200.                " Max Rows
          me->ms_user_settings_xml-maxsel = 200.            " Max Rows
          me->g_user_settings-result_buttons         = abap_true.        " Show Result Buttons
          me->g_user_settings-hd_fieldname           = abap_true.          " Column Header Fieldname
          me->g_user_settings-result_window_vertical = abap_true. " Result Views Vertical            "CDX001-0002
          me->g_user_settings-sql_trace              = abap_true.             " SQL Trace.                       "CDX001-0008
          me->g_user_settings-symbols_show           = abap_false.          " SymbolALV show                   "CDX001-0020
          me->g_user_settings-symbols_program_show   = abap_true.  " Show Program symbols             "CDX001-0020
          me->g_user_settings-only_used_symbols      = abap_false.     " Show only symbols used in Editor "CR22-002
          me->g_user_settings-hd_fieldtext_a         = abap_true.
          me->g_user_settings-history_last_x_days    = 2.                                     " CDX001-0023
          me->g_user_settings-job_last_x_days        = 2.                                         " CDX001-0023
          me->g_user_settings-editor_type            = space.                                             " CDX130-005
      ENDTRY.
    ELSE.
      " fill default values
      me->g_user_settings-use_convexit = abap_true.          " Convers Routine
  ...
  METHOD create_clipboard_ui_control.
    ...
    DATA l_button_data    TYPE stb_button.
    DATA l_stringx        TYPE xstring.
    DATA l_string         TYPE string.
    DATA lt_text          TYPE TABLE OF text255.
    DATA lcl_convin       TYPE REF TO cl_abap_conv_in_ce.
    DATA l_len            TYPE i.
    ...
    gcont_clipboard = gs_splitter_bottom->get_container( row    = 1
                                                         column = 2 ).
    gcont_clipboard->set_name( 'SP_CCONT_B' ).
    " create splitter (textedit & toolbar)
    gs_splitter_clipboard = NEW #( parent  = gcont_clipboard
                                   rows    = 1
                                   columns = 2 ).

    gs_splitter_clipboard->set_name( 'SP_CB' ).
    DATA(lo_element) = gs_splitter_clipboard->get_container( row    = 1
                                                             column = 2 ).
...
*  IF ls_adm_cust-show_element_info = abap_true.
    lo_splitter_text = NEW #( parent  = lo_element
                              rows    = 2
                              columns = 1 ).
    lo_splitter_text->set_name( 'SP_OPTIONS' ).
    gcont_clipboard_textedit = lo_splitter_text->get_container( row    = 1
                                                                column = 1 ).

    gcont_elementinfo = lo_splitter_text->get_container( row    = 2
                                                         column = 1 ).
...
*  ENDIF.
    ...
    gcont_clipboard_toolbar = gs_splitter_clipboard->get_container( row    = 1
                                                                    column = 1 ).

    gs_splitter_clipboard->set_column_mode( cl_gui_splitter_container=>mode_absolute ).
    gs_splitter_clipboard->set_column_width( id    = 1
                                             width = toolbar_col_width ).
    gs_splitter_clipboard->set_column_sash( id    = 1
                                            type  = 1
                                            value = gs_splitter_clipboard->false  ).

    gs_splitter_bottom->set_column_mode( cl_gui_splitter_container=>mode_absolute ).
    gs_splitter_bottom->set_column_width( id    = 2
                                          width = toolbar_col_width ).

    " add splitter to toolbar
    gs_splitter_toolbar = NEW #( parent  = gcont_clipboard_toolbar
                                 rows    = 2
                                 columns = 1 ).
    ...
    gcont_clipboard_toolbar_btns = gs_splitter_toolbar->get_container( row    = 1
                                                                       column = 1 ).
    gcont_clipboard_toolbar_img = gs_splitter_toolbar->get_container( row    = 2
                                                                      column = 1 ).
    ...
    gc_clipboard_toolbar_img = NEW #( parent = gcont_clipboard_toolbar_img ).

    gc_clipboard_toolbar_img->load_picture_from_url( url = url ).

    " create control toolbar clipboard
    gc_clipboard_toolbar = NEW #( parent       = gcont_clipboard_toolbar_btns
    ...
    lt_events = VALUE #( ( eventid    = cl_gui_toolbar=>m_id_function_selected
                           appl_event = ' ' ) ).
    gc_clipboard_toolbar->set_registered_events( events = lt_events ).

    SET HANDLER me->on_toolbar_function_selected FOR gc_clipboard_toolbar.

    " add the buttons to the toolbar
    gc_clipboard_toolbar->add_button_group( data_table = gt_toolbuttons_clipboard ).
    gc_clipboard_toolbar->set_button_visible( visible = ' '
                                              fcode   = 'CLEAR_CLIPBOARD' ).

    " create clipboard text control
    gc_clipboard_textedit = NEW #( parent = gcont_clipboard_textedit ).
    ...
    IF sy-subrc = 0 AND l_stringx IS NOT INITIAL.
      TRY.
          cl_abap_gzip=>decompress_binary( EXPORTING gzip_in = l_stringx
                                           IMPORTING raw_out = l_stringx ).
        CATCH cx_parameter_invalid_range
              cx_sy_buffer_overflow ##NO_HANDLER.
      ENDTRY.

      " convert stringx to string
      lcl_convin = cl_abap_conv_in_ce=>create( ).
    ...
    dragdrop_behaviour_clipboard = NEW #( ).

    dragdrop_behaviour_clipboard->add( flavor     = 'ALV_TO_CLIPBOARD'
                                       dragsrc    = ' '
                                       droptarget = 'X'
                                       effect     = cl_dragdrop=>copy ).

    gc_clipboard_textedit->set_dragdrop( dragdrop = dragdrop_behaviour_clipboard ).
  ...
  METHOD create_controls.
    ...
    IF gc_splitter IS NOT INITIAL.
      RETURN.
    ENDIF.

    create_primary_ui_controls( ).

    create_result_ui_controls( ).

    symbols_controller->create_symbol_ui_control( gs_splitter_top->get_container( row    = 1
                                                                                  column = 2 )  ).

    create_editor_ui_control( ).

    create_clipboard_ui_control( ).

    create_elementinfo_ui_control( ).

    " set focus on abap control (or the abap text control)
    IF gc_abap_editor IS NOT INITIAL.
      gc_abap_editor->set_focus( control = gcont_abap_editor ).
    ELSEIF gc_abap_editor_text IS NOT INITIAL.
      gc_abap_editor_text->set_focus( control = gcont_abap_editor ).
    ENDIF.

    " clear sql area
    usr_action_clear_sql_area( ).

    show_html( ).

    set_result_toolbar_active( i_fcode = c_cmd_home ).

    tippsandtricks( ). " COCKPIT-98
  ENDMETHOD.
  ...
  METHOD create_dyn_document.
    ...
    IF ic_document IS INITIAL.
      ic_document = NEW #( style      = 'ALV_GRID'
                           no_margins = gfw_true ).
      l_reuse = abap_false.
    ELSE.
      ic_document->initialize_document( style      = 'ALV_GRID'
    ...
    IF l_reuse = abap_true.
      ic_document->display_document( reuse_control = l_reuse ).
    ELSE.
      ic_document->display_document( parent = i_parent ).
    ENDIF.
  ENDMETHOD.
  ...
  METHOD create_editor_ui_control.
    ...
    gs_splitter_editor = NEW #( parent     = gcont_abap_splitter
                                rows       = 2
                                columns    = 1
    ...
    gcont_abap_editor = gs_splitter_editor->get_container( row    = 1
                                                           column = 1 ).
    gcont_abap_editor->set_name( |{ c_sqleditor_name }_ED| ).
    gcont_abap_error = gs_splitter_editor->get_container( row    = 2
                                                          column = 1 ).
    gcont_abap_error->set_name( |{ c_sqleditor_name }_ER| ).

    gs_splitter_editor->set_row_height( id     = 2
                                        height = 0 ).
    gs_splitter_editor->set_row_sash( id    = 2
                                      type  = 1
                                      value = gs_splitter_editor->false   ).
    ...
    IF me->g_abap_editor_type = editor_type-new.
      ...
      dragdrop_behaviour_editor = NEW #( ).

      dragdrop_behaviour_editor->add( flavor     = 'ALV_TO_EDITOR'
                                      dragsrc    = ' '
                                      droptarget = 'X'
                                      effect     = cl_dragdrop=>copy ).

      dragdrop_behaviour_editor->add( flavor     = 'LOG_TO_EDITOR'
                                      dragsrc    = ' '
                                      droptarget = 'X'
                                      effect     = cl_dragdrop=>copy ).

      dragdrop_behaviour_editor->add( flavor     = 'SYMBOL_TO_EDITOR'
                                      dragsrc    = ' '
                                      droptarget = 'X'
                                      effect     = cl_dragdrop=>copy ).

      dragdrop_behaviour_editor->add( flavor     = 'ELEMENTINFO_TO_EDITOR'
                                      dragsrc    = ' '
                                      droptarget = 'X'
                                      effect     = cl_dragdrop=>copy ).

      gc_abap_editor->set_dragdrop( dragdrop = dragdrop_behaviour_editor ).

      " register the event
      gc_abap_editor->register_event_dblclick( EXPORTING  navigate_on_dblclick = 1
                                               EXCEPTIONS error_regist_event   = 1
                                                          error_unregist_event = 2 ).

      " context menu
      gc_abap_editor->register_event_context_menu( EXPORTING  register      = 1
                                                              appl_event    = space
                                                              local_entries = 1
                                                   EXCEPTIONS OTHERS        = 6 ).
      ...
      SET HANDLER me->on_editor_dblclick         FOR gc_abap_editor.
      SET HANDLER me->on_editor_context_menu     FOR gc_abap_editor.
      SET HANDLER me->on_editor_context_menu_sel FOR gc_abap_editor.

    ELSE.
      gc_abap_editor_text = NEW #( parent            = gcont_abap_editor
                                   wordwrap_mode     = 2
    ...
    gc_abap_error = NEW #( i_parent = gcont_abap_error ).

    " show error table
    CALL FUNCTION 'LVC_FIELDCATALOG_MERGE'
      EXPORTING i_structure_name = '/CADAXO/SQLCSYNTAXERROR'
    ...
    gc_abap_error->set_table_for_first_display( EXPORTING  i_bypassing_buffer = abap_true
  ...
  METHOD create_elementinfo_ui_control.
    ...
    IF dragdrop_behaviour_elementinfo IS INITIAL.
      dragdrop_behaviour_elementinfo = NEW #( ).
      dragdrop_behaviour_elementinfo->add( flavor     = 'ELEMENTINFO_TO_EDITOR'
                                           dragsrc    = abap_true
    ...
    CALL FUNCTION 'REUSE_ALV_FIELDCATALOG_MERGE'
      EXPORTING  i_structure_name = '/CADAXO/SQLC_ELEMENTINFO'
      CHANGING   ct_fieldcat      = lt_fieldcat
      EXCEPTIONS OTHERS           = 3.
    IF sy-subrc = 0.
    ...
    IF me->g_user_settings-show_footer = abap_true.
      <ls_cont_element>-gui_splitter = NEW #( parent  = <ls_cont_element>-gui_container
                                              rows    = 2
                                              columns = 1 ).
      <ls_cont_element>-gui_splitter->set_name( 'SPLIT' ).
      <ls_cont_element>-gui_splitter->set_border( border = abap_false ).
      ...
      <ls_cont_element>-gui_splitter->set_row_height( id     = 2
                                                      height = toolbar_row_height ).
      <ls_cont_element>-gui_splitter->set_row_sash( id    = 2
                                                    type  = 1
                                                    value = cl_gui_splitter_container=>false  ).

      DATA(lr_cont_main) = <ls_cont_element>-gui_splitter->get_container( row    = 1
                                                                          column = 1 ).
    ...
    gc_elementinfo_alv = NEW #( i_parent = parent_container ).                "++cockpit415
*       i_parent = <ls_cont_element>-gui_container. "--cockpit415

    " Set handler
    SET HANDLER me->on_elementinfo_drag           FOR gc_elementinfo_alv.
    SET HANDLER me->on_elementinfo_double_click   FOR gc_elementinfo_alv.
    ...
    TRY.
        gc_abap_editor->register_event_quick_info( EXPORTING  appl_event = space
                                                   EXCEPTIONS OTHERS     = 1 ).
        "  gc_abap_editor->register_event_f4( EXPORTING appl_event = space
        "                                             EXCEPTIONS OTHERS = 1 ).
        IF sy-subrc = 0.
...
*    gc_docking->set_name( 'GC_DOCKING' ).
    ...
    gc_splitter->set_row_height( id     = 1
                                 height = toolbar_row_height ).

    gc_splitter->set_row_sash( id    = 1
                               type  = 0
                               value = gc_splitter->false  ).
    gcont_splitter_top_toolbar = gc_splitter->get_container( row    = 1
                                                             column = 1 ).
...
*  APPEND ls_stb_button TO gt_toolbuttons_top.                           "COCKPIT-233
    ...
    " add submenues
    l_ctmenu->add_function( fcode   = 'EXECUTE'
                            text    = TEXT-b35
                            checked = abap_true
                            icon    = icon_execute_object ).
    l_ctmenu->add_function( fcode = 'EXECUTEJOB'
                            text  = TEXT-b37 ).
    gc_splitter_top_toolbar->set_static_ctxmenu( fcode   = 'EXECUTE'
                                                 ctxmenu = l_ctmenu ).

    " begin of change 420
    DATA(l_ctmenu2) = NEW cl_ctmenu( ).
    l_ctmenu2->add_function( fcode   = 'SQL_SHARE'
                             text    = TEXT-b41
                             checked = abap_true
                             icon    = icon_workflow_external_event ).
    l_ctmenu2->add_function( fcode = 'SQL_SHR_ME'
                             text  = TEXT-b44 ).
    gc_splitter_top_toolbar->set_static_ctxmenu( fcode   = 'SQL_SHARE'
                                                 ctxmenu = l_ctmenu2 ).
    " end   of change 420

    " begin of change 321
    DATA(l_ctmenu3) = NEW cl_ctmenu( ).
    l_ctmenu3->add_function( fcode   = 'SQLVARSET'
                             text    = TEXT-q40
                             checked = abap_true
                             icon    = icon_alv_variant_save ).
    l_ctmenu3->add_function( fcode    = 'SQLVARSET_UPD'
                             text     = CONV #( TEXT-b46 )
                             disabled = abap_true ).
    ...
    lt_events = VALUE #( appl_event = abap_false
    ...
    gc_splitter->set_row_height( id     = 2
                                 height = toolbar_row_height * 10 ).
    gc_splitter->set_row_sash( id    = 2
                               type  = 1
                               value = gc_splitter->true  ).
...
*  gcont_abap_editor = gc_splitter->get_container( row = 1 column = 1 ).
    ...
    gcont_splitter_top = gc_splitter->get_container( row    = 2
                                                     column = 1 ).
    ...
    gcont_abap_splitter = gs_splitter_top->get_container( row    = 1
                                                          column = 1 ).
    " End Modify
...
*  gc_splitter->set_row_height( id = 2 height = 100 ).
    ...
    gcont_splitter_bottom = gc_splitter->get_container( row    = 3
                                                        column = 1 ). " 3
    ...
    gcont_grid_results = gs_splitter_bottom->get_container( row    = 1
                                                            column = 1 ).
...
*  FIELD-SYMBOLS: <l_button_data> TYPE stb_button.
    ...
    " Result Controls
    gs_splitter_res_button = NEW #( parent     = gcont_grid_results
                                    rows       = 2
                                    columns    = 1
                                    shellstyle = shellstyle ).
    ...
    gs_splitter_res_button->set_row_height( id     = 1
                                            height = toolbar_row_height ). "(24 instead of 20?)
    gs_splitter_res_button->set_row_sash( id    = 1
                                          type  = 1
                                          value = gs_splitter_res_button->false  ).

    gcont_result_toolbar = gs_splitter_res_button->get_container( row    = 1
                                                                  column = 1 ).
    gcont_result_bottom  = gs_splitter_res_button->get_container( row    = 2
                                                                  column = 1 ).
    gcont_result_bottom->set_name( 'GCONT_RESULT_BOTTOM' ).

    " TODO RR:
    IF gc_result_toolbar IS INITIAL.
      gc_result_toolbar = NEW #( parent       = gcont_result_toolbar
                                 display_mode = cl_gui_toolbar=>m_mode_horizontal ).
    ENDIF.
    ...
    gc_result_toolbar->add_button_group( data_table = gt_toolbuttons_result ).
    ...
    gc_result_toolbar->set_registered_events( events = l_events ).
    ...
    gs_splitter_results = NEW #( parent  = gcont_result_bottom
                                 rows    = 1
                                 columns = 1 ).
    ...
    gs_splitter_results->set_row_sash( id    = 1
                                       type  = 1
                                       value = gs_splitter_results->false  ).
    gs_splitter_results->set_column_width( id    = 1
                                           width = 80 ).

    g_cont_pers_preferences = 'X'.

    dragdrop_behaviour_alv = NEW #( ).
  ...
  METHOD create_variant.
    ...
    " get editor
    DATA(l_string) = get_sql_area( ).

    IF l_string IS NOT INITIAL.

      ls_variant-t_sql = get_sql_area_lt_code( ).

      symbols_controller->get_user_symbol_from_sql( EXPORTING i_sql     = ls_variant-t_sql
                                                              i_type    = /cadaxo/cl_sqlc_symbols=>cs_symbol_type-user
                                                    IMPORTING e_symbols = lt_symbols ).
      ls_variant-t_symbol = CORRESPONDING #( lt_symbols ).
*      LOOP AT lt_symbols ASSIGNING FIELD-SYMBOL(<ls_symbol>).
*
*        APPEND CORRESPONDING #( <ls_symbol> ) TO . "COCKPIT-288 Insert
*
*      ENDLOOP.
      ...
      CALL FUNCTION '/CADAXO/SQLC_CREATE_VARIANT_UI'
        ...
        CHANGING  c_vari_name = lv_variant_created. " COCKPIT-321 KA
      ...
      IF lv_variant_created IS NOT INITIAL.
        ...
        l_ctmenu3->add_function( fcode   = 'SQLVARSET'
                                 ...
                                 icon    = icon_alv_variant_save ).
        l_ctmenu3->add_function( fcode    = 'SQLVARSET_UPD'
                                 text     = CONV #( |{ TEXT-b46 } { lv_variant_created }| )
                                 disabled = abap_false ).
    ...
    TRY.
        ...
        LOOP AT gt_cl_sql_parse ASSIGNING <lr_cl_sql_parse>.
          ...
          IF <lr_cl_sql_parse>->g_select_version = <lr_cl_sql_parse>->c_select_version_1.
            lt_lvc_t_fcat = <lr_cl_sql_parse>->create_alv_field_catalog( i_user_settings   = g_user_settings
          ...
          " COCKPIT-458 BEGIN
          IF <lr_cl_sql_parse>->g_select_version = /cadaxo/cl_sqlc_cockpit_parse=>c_select_version_2.
            IF <lr_cl_sql_parse>->g_main_ref->g_user_settings-domaintext = abap_true.
              <lr_cl_sql_parse>->add_domain_value( ).
              gt_lvc_t_fcat[ l_index_sql ] = <lr_cl_sql_parse>->gt_lvc_t_fcat.
            ENDIF.
          ENDIF.
          " COCKPIT-458 END
          ...
          CALL FUNCTION 'SAPGUI_PROGRESS_INDICATOR'
            EXPORTING text = TEXT-p02. " The data are formatted for output

          IF l_error_message IS INITIAL.
            g_result_layout-grid_title = /cadaxo/cl_sqlc_cockpit_main=>build_result_grid_title(
                                             i_runtime = l_result_details-runtime
                                             i_lines   = l_result_details-lines ).
          ELSE.
            g_result_layout-grid_title = l_error_message.
          ...
          IF me->mv_toolbar_result_active <> c_cmd_show_result_table.
            set_result_toolbar_active( i_fcode = c_cmd_show_result_table ).
          ENDIF.
        ENDLOOP.
...
*          l_message = 'EXC!'.      "-Cockpit-374
    ...
    IF l_message IS NOT INITIAL.
      ...
      handle_msg_exception(                  " COCKPIT-269
                            i_msg       = l_message           " COCKPIT-269
                            i_exception = lr_exception ). " COCKPIT-269; COCKPIT-269
...
*    ENDIF.
  ...
  METHOD execute_sql_background_wiz.
    ...
    TRY.
        ...
        " check sql syntax
        check_sql_syntax( i_use_local_parser = abap_true ).

        " check free space for the job
        /cadaxo/cl_sqlc_cockpit_assist=>get_adm_customizing( IMPORTING e_customizing = ls_adm_cust ).

        IF ls_adm_cust-maxspace > 0.
        ...
        ELSE.                                                                              " COCKPIT-7
          gc_abap_editor_text->get_text_as_r3table( IMPORTING table = lt_code ).       " COCKPIT-7
        ENDIF.                                                                             " COCKPIT-7
...
*  "COCKPIT-488 DEL END
  ...
  METHOD get_content.
    ...
    DATA l_doc_url          TYPE c LENGTH 80.
    DATA l_size             TYPE i.
    ...
    IF <lt_html_cache> IS INITIAL.
      ...
      /cadaxo/cl_sqlc_cockpit_assist=>get_parameter_value( EXPORTING  i_parameter_id      = l_html_id_langu
                                                           RECEIVING  r_parameter_value   = l_par_value
                                                           EXCEPTIONS parameter_not_found = 1
                                                                      OTHERS              = 2 ).
      IF sy-subrc <> 0.
        /cadaxo/cl_sqlc_cockpit_assist=>get_parameter_value( EXPORTING  i_parameter_id      = l_html_id
                                                             RECEIVING  r_parameter_value   = l_par_value
                                                             EXCEPTIONS parameter_not_found = 1
                                                                        OTHERS              = 2 ).
        IF sy-subrc <> 0 AND i_viewer IS SUPPLIED AND i_viewer IS BOUND.
          ...
          i_viewer->load_mime_object( EXPORTING  object_id    = l_html_objid_langu
          ...
          IF sy-subrc <> 0.
            ...
            i_viewer->load_mime_object( EXPORTING  object_id    = l_html_objid
                                                   object_url   = l_doc_url
                                        IMPORTING  assigned_url = l_doc_url
                                        EXCEPTIONS OTHERS       = 1 ).
          ENDIF.
      ...
      IF sy-subrc = 0.
        ...
        param_replace_tags( CHANGING data = l_par_value ).
          ...
          LOOP AT <lwa_result>-submatches ASSIGNING <lwa_sub> WHERE offset > 0 AND length > 0.
            ...
            IF l_doc_url IS INITIAL.
              ...
              ELSE.
                CLEAR l_mime.
              ENDIF.
              get_content( EXPORTING i_html_id      = l_html_id
                                     i_viewer       = i_viewer
                                     i_mime         = l_mime
                           IMPORTING et_content     = lt_subhtml
                                     e_size         = l_size
  ...
  METHOD get_saved_list_fieldcat.
    ...
    CALL FUNCTION 'REUSE_ALV_FIELDCATALOG_MERGE'
      EXPORTING  i_structure_name       = '/CADAXO/SQLCSRESALV'
      CHANGING   ct_fieldcat            = lt_fieldcat
    ...
    IF sy-subrc = 0.
      ...
      LOOP AT lt_fieldcat ASSIGNING FIELD-SYMBOL(<l_fieldcat>).
        ...
        l_lvc_s_fcat-scrtext_m = <l_fieldcat>-seltext_m.
        l_lvc_s_fcat-scrtext_l = <l_fieldcat>-seltext_l.
        l_lvc_s_fcat-scrtext_s = <l_fieldcat>-seltext_s.
  ...
  METHOD get_saved_results.
    ...
    LOOP AT i_ress_guid INTO ls_ress_guid.
      ...
      IF sy-subrc = 0.
        me->prepare_result_table( is_sqlcsres = ls_sqlcsres
  ...
  METHOD get_selected_elem_inf_flds.
    ...
    gc_elementinfo_alv->get_selected_rows( IMPORTING et_index_rows = DATA(selected_rows) ).

    IF lines( selected_rows ) > 0.
      LOOP AT selected_rows ASSIGNING FIELD-SYMBOL(<selected_row>).
        ASSIGN gt_elementinfo[ <selected_row>-index ] TO FIELD-SYMBOL(<ls_elementinfo>).
        IF sy-subrc <> 0.
          CONTINUE.
        ENDIF.
    ...
    ELSE.
      ...
      ASSIGN gt_elementinfo[ i_index ] TO <ls_elementinfo>.
      IF sy-subrc = 0.
        IF <ls_elementinfo>-alias IS INITIAL.
  ...
  METHOD get_sql_area.
    ...
    DATA l_pt       TYPE i.
    DATA lt_code    TYPE /cadaxo/sqlccodeline_t.
    DATA l_code     TYPE /cadaxo/sqlccodeline.
    DATA l_match    TYPE i.
    DATA lt_results TYPE match_result_tab.
    ...
    FIELD-SYMBOLS <l_code2>    LIKE LINE OF lt_code.
    FIELD-SYMBOLS <l_results>  LIKE LINE OF lt_results.
  ...
  METHOD get_sql_hist_lines.
    r_sql_hist_lines = LINES( gt_sql_hist ).
  ENDMETHOD.
  ...
  METHOD get_variant.
    ...
    CALL FUNCTION '/CADAXO/SQLC_MAINTAINT_VAR_UI'
      EXPORTING i_mode       = 'G'
      IMPORTING e_il_variant = gs_sel_variant.

    IF gs_sel_variant IS NOT INITIAL.

      " set sql editor lines
      set_sql_area( gs_sel_variant-t_sql  ).

      l_sqlcvari = CORRESPONDING #( gs_sel_variant ).

      l_sqlcvari-username = gs_sel_variant-cruser.

      symbols_controller->get_user_symbol_from_sql(
        EXPORTING i_varguid = gs_sel_variant-varguid
                  i_sql     = gs_sel_variant-t_sql
                  i_type    = /cadaxo/cl_sqlc_symbols=>cs_symbol_type-variant
        IMPORTING e_symbols = lt_symbols ).
      IF lt_symbols IS NOT INITIAL.

        symbols_controller->merge_symbols( lt_symbols ).
  ...
  METHOD handle_command_show_full_value.
    ...
    IF sy-subrc = 0.
      ...
      CALL FUNCTION '/CADAXO/SQLC_UT_VALUE_POPUP'
        EXPORTING iv_edit  = abap_false
        CHANGING  cv_value = <result_field>.
    ENDIF.
  ENDMETHOD.
  ...
  METHOD handle_command_show_html_brow.
    ...
    IF <result_field> IS ASSIGNED.
      ...
      cl_abap_browser=>show_html( title        = TEXT-t15
...
*      ).
  ...
  METHOD handle_command_show_xml_brow.
    ...
    IF sy-subrc = 0.
      ...
      IF <result_field> IS ASSIGNED.
        ...
        cl_abap_browser=>show_xml( xml_string   = CONV #( <result_field> )
                                   title        = TEXT-t16
                                   size         = cl_abap_browser=>large
                                   modal        = abap_true
                                   printing     = abap_true
  ...
  METHOD handle_delete_saved_lists.
    ...
    ASSIGN gcont_grid_result_t[ 1 ] TO FIELD-SYMBOL(<l_cont_grid_result>).
    IF sy-subrc <> 0.
      RETURN.
    ENDIF.

    <l_cont_grid_result>-gui_alv_grid->get_selected_rows( IMPORTING et_row_no = lt_lvc_t_roid ).

    IF lt_lvc_t_roid[] IS NOT INITIAL.
      ...
      IF l_rc = '1'.
        ...
        LOOP AT lt_lvc_t_roid ASSIGNING FIELD-SYMBOL(<l_lvc_s_roid>).
          ASSIGN me->gt_saved_lists[ <l_lvc_s_roid>-row_id ] TO FIELD-SYMBOL(<ls_saved_lists>).
          IF sy-subrc = 0.
            /cadaxo/cl_sqlc_cockpit_lists=>delete_list( i_list_guid = <ls_saved_lists>-list_guid
        ...
        /cadaxo/cl_sqlc_cockpit_lists=>get_saved_lists( EXPORTING i_uname       = sy-uname
                                                        IMPORTING e_saved_lists = gt_saved_lists ).
  ...
  METHOD handle_export_saved_list.
    ...
    l_lines = LINES( lt_lvc_t_roid ).

    IF l_lines = 1.
      ASSIGN lt_lvc_t_roid[ 1 ] TO FIELD-SYMBOL(<l_lvc_s_roid>).
      IF sy-subrc = 0.
        ASSIGN me->gt_saved_lists[ <l_lvc_s_roid>-row_id ] TO FIELD-SYMBOL(<ls_saved_lists>).
        IF sy-subrc = 0.
          /cadaxo/cl_sqlc_cockpit_lists=>export_list( i_list_guid = <ls_saved_lists>-list_guid ).
        ENDIF.
      ENDIF.
    ENDIF.
  ENDMETHOD.
  ...
  METHOD handle_msg_exception.
    ...
    gc_abap_error->refresh_table_display( ).
    gs_splitter_editor->set_row_height( id     = 2
                                        height = /cadaxo/cl_sqlc_resolution=>get_instance( )->calculate_error_height(
                                                     i_number_of_errors = lines( gt_errors ) ) ).
  ENDMETHOD.
  ...
  METHOD handle_result_command_close.
    ...
    ENDIF.
    IF gcont_grid_result_t[] IS NOT INITIAL.
      show_result( ).
    ELSE.
      set_result_toolbar_active( i_fcode = c_cmd_home ).
      show_html( ).
...
*    FIELD-SYMBOLS: <l_grid>           LIKE LINE OF me->gcont_grid_result_t.
    ...
    ASSIGN me->dref_result_tab_t[ i_source ] TO <lr_source>.
    ASSIGN me->dref_result_tab_t[ i_target ] TO <lr_target>.

    ASSIGN <lr_source>->* TO <lt_source>.
    ASSIGN <lr_target>->* TO <lt_target>.

    ASSIGN me->gt_cl_sql_parse[ i_source ] TO <ls_sql_parse_s>.
    ASSIGN me->gt_cl_sql_parse[ i_target ] TO <ls_sql_parse_t>.
    ...
    CALL FUNCTION '/CADAXO/SQLCDCOMPCOMPLEX'
      EXPORTING it_source             = <lt_source>
                ...
                i_user_settings       = g_user_settings
...
*    ENDIF.
  ...
  METHOD handle_result_command_fulldisp.
    ...
    "  Date       | Developer            | Description                                 |                -
    " ------------+----------------------+---------------------------------------------+-----------------
    "  18.02.2017 | Domi Bigl            | Conversion Exits and Fieldnames             | COCKPIT-105    -
    " ------------+----------------------+---------------------------------------------+-----------------
    ...
    FIELD-SYMBOLS <ls_fcat_main>  TYPE lvc_s_fcat.
    ...
    " TODO: variable is assigned but never used (ABAP cleaner)
    ASSIGN gt_cl_sql_parse[ i_grid_i ] TO FIELD-SYMBOL(<lr_parser>). " COCKPIT-105

    ASSIGN <lr_dref>->* TO <lt_result_tab>.

    IF <lt_result_tab> IS NOT ASSIGNED.
      RETURN.
    ENDIF.
    ...
    " get the list header
    ASSIGN gcont_grid_result_t[ i_grid_i ] TO FIELD-SYMBOL(<lr_grid>).
    IF sy-subrc = 0.

      <lr_grid>-gui_alv_grid->get_frontend_layout( IMPORTING es_layout = ls_layout ).

      lr_display_settings->set_list_header( value = ls_layout-grid_title ).
  ...
  METHOD handle_result_command_hold.
    ...
    FIELD-SYMBOLS <lr_sql>             LIKE LINE OF gt_cl_sql_parse.
    FIELD-SYMBOLS <l_cont_grid_result> LIKE LINE OF me->gcont_grid_result_t.

    ASSIGN dref_result_tab_t[ i_grid_i ] TO <lr_dref>.
    IF sy-subrc = 0.

      l_index = sy-tabix.

      " set hold flag in parser class.
      ASSIGN gt_cl_sql_parse[ l_index ] TO <lr_sql>.
      IF sy-subrc = 0.
      ...
      ASSIGN me->gcont_grid_result_t[ l_index ] TO <l_cont_grid_result>.
      IF sy-subrc = 0.
  ...
  METHOD handle_result_command_keyfix.
    ...
    FIELD-SYMBOLS <lr_sql>             LIKE LINE OF gt_cl_sql_parse.
    FIELD-SYMBOLS <l_cont_grid_result> LIKE LINE OF me->gcont_grid_result_t.
    FIELD-SYMBOLS <l_lvc_t_fcat>       LIKE LINE OF gt_lvc_t_fcat.
    ...
    IF sy-subrc = 0.
      ...
      IF sy-subrc = 0.
        ...
        ASSIGN gt_cl_sql_parse[ l_index ] TO <lr_sql>.
        IF sy-subrc = 0.
          ASSIGN gt_lvc_t_fcat[ l_index ] TO <l_lvc_t_fcat>.
          IF sy-subrc = 0.
            ...
            <l_cont_grid_result>-gui_alv_grid->set_table_for_first_display(
              EXPORTING  i_bypassing_buffer            = 'X'
                         ...
                         it_toolbar_excluding          = g_result_toolbar_excluding
              CHANGING   it_fieldcatalog               = <l_lvc_t_fcat>
                         it_outtab                     = <lt_result>
  ...
  METHOD handle_result_command_refrlst.
    ...
    IF sy-subrc = 0.
      ...
      IF sy-subrc = 0.
        ...
        " get result details
        ASSIGN me->gt_result_details[ l_index ] TO <ls_result_details>.
        IF sy-subrc = 0.
          ...
          IF sy-subrc = 0.
            ...
            lv_footer = /cadaxo/cl_sqlc_cockpit_main=>build_result_grid_footer(
...
*End   of Comments Cockpit-398
    ...
    cl_gui_frontend_services=>file_save_dialog( EXPORTING  default_extension = 'csv'
                                                           file_filter       = '.csv'
    ...
    IF sy-subrc = 0 AND lv_user_action = 0.
      ...
      cl_gui_frontend_services=>gui_download( EXPORTING  filename = lv_fullpath
                                              CHANGING   data_tab = iv_output_table
                                              EXCEPTIONS OTHERS   = 1 ).
      IF sy-subrc <> 0.
        MESSAGE ID sy-msgid TYPE sy-msgty NUMBER sy-msgno
                WITH sy-msgv1 sy-msgv2 sy-msgv3 sy-msgv4.
  ...
  METHOD insert_codeblock_currpos_nosel.
    ...
    IF    l_from_line <> l_to_line
       OR l_from_pos  <> l_to_pos.

      MESSAGE i005(/cadaxo/sqlc).

    ELSE.

      insert_codeblock_at_position( iv_line      = l_from_line
                                    iv_pos       = l_from_pos
                                    iv_sqlstring = iv_sqlstring
  ...
  METHOD insert_saved_list.
    ...
    CALL FUNCTION 'GUID_CREATE'
      IMPORTING ev_guid_16 = ls_sqlcress-ress_guid.

    ls_sqlcsres = CORRESPONDING #( ls_saved_list ).
    ls_sqlcsres-ress_guid = ls_sqlcress-ress_guid.
    CALL FUNCTION 'GUID_CREATE'
      IMPORTING ev_guid_16 = ls_sqlcsres-list_guid.
  ...
  METHOD insert_table_to_editor.
    ...
    insert_codeblock_currpos_nosel( i_string ).
  ENDMETHOD.

  METHOD is_result_filled.
    IF dref_result_tab_t[] IS NOT INITIAL.
      r_filled = abap_true.
    ENDIF.
  ENDMETHOD.
  ...
  METHOD load_home_html.
    ...
    DATA l_parameter_val   TYPE /cadaxo/sqlcparameter_val.
    ...
    FIELD-SYMBOLS <l_file>      TYPE string.
    ...
    cl_gui_frontend_services=>file_open_dialog(
      ...
      CHANGING   file_table              = lt_file_name
                 rc                      = l_rc
    ...
    IF sy-subrc = 0.
      CASE l_user_action.
        WHEN 9. " Cancelled by user
        WHEN 0. " File selected
          ASSIGN lt_file_name[ 1 ] TO <l_file_name>.
          IF sy-subrc = 0.
            l_filename_string = <l_file_name>-filename.

            " call gui upload dialog
            cl_gui_frontend_services=>gui_upload( EXPORTING  filename                = l_filename_string
            ...
            IF sy-subrc = 0.
              ...
              /cadaxo/cl_sqlc_cockpit_assist=>set_parameter_value( i_parameter_id    = l_param_id
  ...
  METHOD log_alv_line_selection.
    ...
    gui_alv_grid->refresh_table_display( EXPORTING  i_soft_refresh = abap_true
    ...
    gv_selected_counter += 1.

    TRY.
        gui_alv_grid->set_selected_rows( it_index_rows            = gt_selected_disp
                                         is_keep_other_selections = abap_true ).
        MESSAGE s137(/cadaxo/sqlc) WITH gv_selected_counter gv_selected_total.
        IF gt_selected_rows IS INITIAL.
          CLEAR: gv_selected_counter,
                 gv_selected_total.
        ENDIF.
  ...
  METHOD match_saved_fieldcat_orig.
    ...
    "  Date       | Developer            | Description                                 |                -
    " ------------+----------------------+---------------------------------------------+-----------------
    "  11.09.2014 | Dieter Schadler      | Method completely rewritten                  | ticket #138    -
    " ------------+----------------------+---------------------------------------------+-----------------
    "  06.08.2015 | Ana Lekic            | no output from the 100th field -  suppress   | RT340          -
    " ------------+----------------------+---------------------------------------------+-----------------
    ...
    LOOP AT lt_fcat ASSIGNING <fs_fcat>.
      ...
      ASSIGN lt_tabledetails[ tabname = lv_tabname ] TO <ls_tabledetails>.
      IF sy-subrc <> 0.
        ...
        CALL FUNCTION 'LVC_FIELDCATALOG_MERGE'
          ...
          CHANGING   ct_fieldcat            = <ls_tabledetails>-it_fcat_ddic
          EXCEPTIONS inconsistent_interface = 1
                     program_error          = 2
                     OTHERS                 = 3.
        IF sy-subrc <> 0.
      ...
      ASSIGN <ls_tabledetails>-it_fcat_ddic[ fieldname = lv_fieldname ] TO <fs_fcat_ddic>.
      IF sy-subrc <> 0.
  ...
  METHOD move_back_to_sql.
    ...
    IF gt_sql_hist[] IS NOT INITIAL.

      g_sql_pos -= 1.

      ASSIGN gt_sql_hist[ g_sql_pos ] TO <l_sqlchistline>.
      IF sy-subrc = 0.

        usr_action_clear_sql_area( ).

        set_sql_area( i_codelines_t = <l_sqlchistline>-lines[]  ).
  ...
  METHOD move_forw_to_sql.
    ...
    IF gt_sql_hist[] IS NOT INITIAL.
      ...
      l_sql_pos = g_sql_pos + 1.

      ASSIGN gt_sql_hist[ l_sql_pos ] TO <l_sqlchistline>.
      IF sy-subrc = 0.
  ...
  METHOD on_abap_error_hotspot_click.
    ...
    ASSIGN gt_errors[ e_row_id ] TO <ls_error>.
    IF sy-subrc <> 0.
      RETURN.
    ENDIF.
    ...
    ELSE.
      ...
      CALL FUNCTION 'HELP_START'
        EXPORTING help_infos   = ls_help_info
        TABLES    dynpselect   = lt_dselc
  ...
  METHOD on_alv_drag.
    ...
    DATA l_typ             TYPE c LENGTH 1.
    DATA l_fieldvalue      TYPE string.
    DATA l_fieldvaluec     TYPE c LENGTH 150.
    DATA lr_cl_gui_control TYPE REF TO cl_gui_control.
    DATA l_grid_name       TYPE string.
    DATA l_grid_name_i     TYPE i.
    DATA l_fieldname_txt   TYPE c LENGTH 30.
    ...
    FIELD-SYMBOLS <lr_dref>         TYPE REF TO data.
    FIELD-SYMBOLS <lr_cl_sql_parse> TYPE REF TO /cadaxo/cl_sqlc_cockpit_parse.
    FIELD-SYMBOLS <ls_sql_dfies>    TYPE /cadaxo/sqlcdfies.
    ...
    ASSIGN <lt_result_tab>[ e_row-index ] TO <l_result_line>.
    IF sy-subrc <> 0.
      RETURN.
    ENDIF.
    ...
    IF l_fieldname_txt IS INITIAL.                                " CDX001-0026
      l_fieldname_txt = e_column-fieldname.                 " CDX001-0026
    ENDIF.                                                        " CDX001-0026
    ...
    CASE e_dragdropobj->flavor.
      ...
      WHEN 'ALV_TO_EDITOR'.
        ...
        TRY.
            l_fieldvalue = /cadaxo/cl_sqlc_cockpit_assist=>convert_value_int_to_editor(
                               iv_typ                 = l_typ
                               iv_fieldvalue_internal = <l_result_field> ).
          CATCH /cadaxo/cx_sqlc_cockpit_assist INTO DATA(lr_sqlc_exception). " TODO: variable is assigned but never used (ABAP cleaner)
...
* end   of change COCKPIT-294
    ...
    DATA l_rc       TYPE c LENGTH 1. " COCKPIT-383

    " read selected template
    ASSIGN gt_queue[ e_row-index ] TO FIELD-SYMBOL(<ls_queue_alv>).
    IF sy-subrc = 0.
      ...
      LOOP AT lt_items ASSIGNING FIELD-SYMBOL(<ls_items>).
        ...
        ELSEIF <ls_items>-typ = /cadaxo/cl_sqlc_cockpit_api=>cs_api_types-symbols.
          ...
          IF lt_symbols IS NOT INITIAL. " check symbols and give pop-up if identical data exists
            symbols_controller->merge_symbols( lt_symbols ).
          ENDIF.
          " end   of change COCKPIT-294
        ...
        ELSEIF <ls_items>-typ = /cadaxo/cl_sqlc_cockpit_api=>cs_api_types-savedlist.
          api_saved_list_import( ir_api   = lr_api
                                 is_items = <ls_items> ).     " cockpit-401
      ...
      IF lt_sql IS NOT INITIAL.
        ...
        DATA(lt_sql_area) = get_sql_area_lt_code( ).
        IF     <ls_queue_alv>-position_typsql  = /cadaxo/cl_sqlc_cockpit_api=>cs_api_types-sql
           AND lt_sql_area                    IS NOT INITIAL.
          CALL FUNCTION 'POPUP_TO_CONFIRM'
            EXPORTING  titlebar              = TEXT-t17
...
* CHECK NOT g_user_settings-result_doubleclick IS INITIAL.                         "FOE28012011
    ...
    cl_gui_alv_grid=>get_focus( IMPORTING control = l_cl_gui_control ).
    ...
    IF e_row-index = 0.                                    " CDX001-0006
      ...
      IF sy-subrc = 0.
        ...
        ELSE.
          ...
          ASSIGN me->gt_cl_sql_parse[ l_grid_name_i ] TO <lr_cl_sql_parse>.
          IF sy-subrc = 0.
            CASE <lr_cl_sql_parse>->g_select_version.
              WHEN <lr_cl_sql_parse>->c_select_version_0 OR <lr_cl_sql_parse>->c_select_version_1.

                ASSIGN <lr_cl_sql_parse>->gt_result_ddfields[ map_fieldname = e_column-fieldname ] TO <ls_sql_dfies>.
                ...
                IF <ls_sql_dfies> IS ASSIGNED.
                  ...
                  IF l_fieldvalue CO l_compare_guid AND <ls_sql_dfies>-fieldname <> space.
                    l_fieldvalue = <ls_sql_dfies>-fieldname.
                  ENDIF.
                ENDIF.

              WHEN <lr_cl_sql_parse>->c_select_version_2.

                IF line_exists( <lr_cl_sql_parse>->gt_result_ddfields[ fieldname = l_dummy ] ).
                  l_fieldvalue = |{ l_dummy }~{ l_fieldvalue }|.
                ELSE.
                  IF l_fieldvalue IS INITIAL.
                    l_fieldvalue = l_dummy.
                  ENDIF.
                  ASSIGN <lr_cl_sql_parse>->gt_result_ddfields[ fieldname = l_fieldvalue ] TO <ls_sql_dfies>.
    ...
    ELSE.                                                   " CDX001-0006
      ...
      IF sy-subrc = 0.
        ...
        insert_codeblock_currpos_nosel( l_fieldvalue ).
  ...
  METHOD on_alv_templ_double_click_2000.
    ...
    SELECT SINGLE template_class FROM /cadaxo/sqlctemp
      INTO l_sqlctemp_class
      WHERE template_name = <l_sql_template_alv>-template_name.
    IF sy-subrc = 0.
  ...
  METHOD on_clipboard_drop.
    DATA l_text_line   TYPE c LENGTH 256.
    ...
    TRY.
        ...
        ENDIF.
        gc_clipboard_textedit->set_textstream( text = l_text_string ).
  ...
  METHOD on_editor_context_menu.
    ...
    DATA lr_submenu_symbols TYPE REF TO cl_ctmenu.

    menu->add_function( fcode = c_cmd_insert_table
                        text  = TEXT-b04 ).
    menu->add_function( fcode = c_cmd_insert_cds_entity
                        text  = TEXT-b38 ).
    menu->add_function( fcode = c_cmd_insert_sy_field
                        text  = TEXT-b05 ). " CDX25012010
    "    menu->add_function( fcode = c_cmd_insert_cc         text = text-b49 ). "COCKPIT-474 'Insert Code Completion'
    menu->add_function( fcode = c_cmd_insert_header
                        text  = TEXT-b50 ). " COCKPIT-472 'Insert Header
    menu->add_function( fcode = c_cmd_pp
                        text  = TEXT-b34 ).  " COCKPIT-260

    lr_submenu_symbols = NEW #( ).

    menu->add_submenu( menu = lr_submenu_symbols
                       text = TEXT-b11 ).

    lr_submenu_symbols->add_function( fcode = 'INSERT_DYN_SYMB'
                                      text  = TEXT-b12 ).
    lr_submenu_symbols->add_function( fcode    = 'INSERT_USR_SYMB'
                                      text     = TEXT-b13
                                      disabled = 'X' ).

    lr_submenu = NEW #( ).

    menu->add_submenu( menu = lr_submenu
                       text = TEXT-b10 ).

    lr_submenu->add_function( fcode = 'COPY_TO_X_BUFFER'
                              text  = TEXT-b07 ).
    lr_submenu->add_function( fcode = 'COPY_TO_Y_BUFFER'
                              text  = TEXT-b08 ).
    lr_submenu->add_function( fcode = 'COPY_TO_Z_BUFFER'
                              text  = TEXT-b09 ).
  ENDMETHOD.
  ...
  METHOD on_editor_context_menu_sel.
    ...
    DATA l_string     TYPE string.
    DATA l_buffer_key TYPE c LENGTH 22.
    DATA lt_code      TYPE /cadaxo/sqlccodeline_t.
    DATA lt_buffer    TYPE STANDARD TABLE OF string.

    FIELD-SYMBOLS <ls_code>   LIKE LINE OF lt_code.
    FIELD-SYMBOLS <ls_buffer> LIKE LINE OF lt_buffer.

    CASE fcode.
      WHEN c_cmd_insert_cds_entity. " CDS Views
        /cadaxo/cl_sqlc_cockpit_assist=>value_help_cds_views( IMPORTING e_cds_view = l_string ).
        IF sy-subrc = 0 AND l_string IS NOT INITIAL.
          insert_table_to_editor( i_string = l_string ).
        ENDIF.
      WHEN 'INSERT_DYN_SYMB'. " dynamic symbols
        /cadaxo/cl_sqlc_cockpit_assist=>value_help_dyn_symbols( IMPORTING e_symbol_name = l_string ).
        IF sy-subrc = 0 AND l_string IS NOT INITIAL.
          CONCATENATE '&' l_string '&' INTO l_string.
          insert_table_to_editor( i_string = l_string ).
        ENDIF.
      WHEN c_cmd_insert_table.    " insert table
        /cadaxo/cl_sqlc_cockpit_assist=>value_help_dd_table( IMPORTING  e_tabname         = l_tabname
...
*        ENDIF.
      WHEN c_cmd_insert_header. " COCKPIT-472
        /cadaxo/cl_sqlc_cockpit_assist=>insert_select_header( CHANGING co_abap_editor = gc_abap_editor ).
      ...
      WHEN c_cmd_insert_sy_field. " insert system fields                    "CDX25012010
        ...
        IF sy-subrc = 0.                                              " CDX25012010
          insert_table_to_editor( i_string = l_string ). " CDX25012010
        ENDIF.                                                         " CDX25012010
  ...
  METHOD on_editor_dblclick.
    ...
    DATA l_to_pos     TYPE i.
    DATA lt_code      TYPE /cadaxo/sqlccodeline_t.
    DATA l_stringc    TYPE char255.
    DATA l_len        TYPE i.
    DATA l_ddobjname  TYPE rsedd0-ddobjname.
    ...
    ASSIGN lt_code[ l_from_line ] TO <l_code>.
    IF sy-subrc <> 0.
      RETURN.
    ENDIF.
    ...
    CALL FUNCTION 'DDIF_TYPEINFO_GET'
      EXPORTING typename = l_ddobjname
      IMPORTING typekind = l_ddtypekind.
  ...
  METHOD on_editor_drop.
    ...
    insert_codeblock_at_position( iv_line      = line
                                  iv_pos       = pos
  ...
  METHOD on_editor_insert_pattern.
    ...
    " get insertion text
    lr_parser->calculate_insertion_result( EXPORTING kind           = datatype
    ...
    IF success = abap_true.
      ...
      sender->select_range( EXPORTING  from_line = ypos
                                       from_pos  = xpos
                                       to_line   = ypos
                                       to_pos    = xpos
                            EXCEPTIONS OTHERS    = 1 ).
      ...
      sender->set_selected_text_as_table( table = compl_text ).

      " Refocus editor window (req'd for 710 GUI)
      sender->set_focus( control = sender ).
    ENDIF.
  ENDMETHOD.
  ...
  METHOD on_editor_quick_info.
    ...
    lr_parser->calculate_completion_results( EXPORTING  ypos           = ypos
                                                        xpos           = xpos
                                                        beg_ypos       = ypos
                                                        beg_xpos       = 1
                                             ...
                                             CHANGING   incl_source    = lt_source
                                             EXCEPTIONS OTHERS         = 1 ).
    IF sy-subrc <> 0.
      RETURN.
    ENDIF.
    ...
    IF xpos >= l_to OR contextstring+xpos(1) = space.
      RETURN.
    ENDIF.
...
*     l_from = l_xpos + 1.  "-cockpit433
    ...
    IF contextstring CA '\'.
      /cadaxo/cl_sqlc_cockpit_assist=>get_cds_view_of_association( EXPORTING iv_association = contextstring
    ...
    IF lv_typekind = 'T'.
      ...
      CALL FUNCTION 'DDIF_FIELDINFO_GET'
        EXPORTING  tabname   = CONV tabname( lv_entity )
      ...
      LOOP AT gt_elementinfo ASSIGNING FIELD-SYMBOL(<ls_elemet_info>).
        ...
        ELSEIF <ls_elemet_info>-decimals IS INITIAL.
          <ls_elemet_info>-data_type = |{ to_lower( <ls_elemet_info>-datatype  ) }({ shift_left(
                                                                                         val = <ls_elemet_info>-leng
                                                                                         sub = '0' ) })|.
        ELSE.
          <ls_elemet_info>-data_type = |{ to_lower( <ls_elemet_info>-datatype ) }({ shift_left(
                                                                                        val = <ls_elemet_info>-leng
      ...
      IF me->g_user_settings-show_footer = abap_true AND gcont_grid_elementinfo_t[] IS NOT INITIAL.
        ...
        DATA(lr_cont_footer) = <ls_cont_element>-gui_splitter->get_container( row    = 2
                                                                              column = 1 ).
        ...
        create_dyn_document( EXPORTING i_parent      = lr_cont_footer
                                       i_sql         = space
                                       i_header_text = lv_footer
                             CHANGING  ic_document   = <ls_cont_element>-cl_document_footer ).
      ENDIF.
      " end of change+cockpit415
    ...
    ELSEIF lv_typekind = 'B'.
      ...
      IF lines( lt_entity ) > 0.
        ...
        lr_handler->if_dd_ddl_handler~get_ddl_content_object_names( EXPORTING ddlname    = g_ddlname
                                                                    IMPORTING viewname   = g_viewname
                                                                              entityname = g_entityname ).

        lr_sobject = cl_dd_sobject_factory=>create( ).
        APPEND g_entityname TO lt_sobjnames.

*{   REPLACE        A4HK9F001G                                        2
*\          lr_sobject->read(
*\            EXPORTING
*\              get_state      = 'M'
...
*\              dd03ndv_tab    = lt_dd03ndvtab ).
        ...
        lr_sobject->read( EXPORTING get_state   = 'M'
                                    sobjnames   = lt_sobjnames
                          IMPORTING dd03ndv_tab = lt_dd03ndvtab
                                    dd08bv_tab  = dd08bv_tab
                                    dd05bv_tab  = dd05bv_tab
                                    dd05fv_tab  = dd05fv_tab ).
        " }   REPLACE
        ...
        LOOP AT lt_dd03ndvtab ASSIGNING FIELD-SYMBOL(<dd03>).
          ...
          ELSEIF <dd03>-decimals IS INITIAL.
            ls_element_info-data_type = |{ to_lower( <dd03>-datatype  ) }({ shift_left( val = <dd03>-leng
                                                                                        sub = '0' ) })|.
          ELSE.
            ls_element_info-data_type = |{ to_lower( <dd03>-datatype ) }({ shift_left( val = <dd03>-leng
                                                                                       sub = '0' ) }|
                                                                            && |,{ shift_left( val = <dd03>-decimals
                                                                                               sub = '0' ) })|.
          ENDIF.
  ...
  METHOD on_elementinfo_double_click.
    ...
    CHECK g_user_settings-result_doubleclick IS NOT INITIAL.
    ...
    l_fields = get_selected_elem_inf_flds( i_index = e_row-index ).

    IF l_fields IS NOT INITIAL.

      insert_codeblock_currpos_nosel( l_fields ).
  ...
  METHOD on_elementinfo_hotspot_de.
    CALL FUNCTION 'RS_DD_SHOW'
      EXPORTING  objname              = gt_elementinfo[ e_row_id ]-rollname
                 objtype              = 'E'
      EXCEPTIONS object_not_found     = 1
                 object_not_specified = 2
                 permission_failure   = 3
  ...
  METHOD on_handle_job_toolbar.
    ...
    " release button
    CLEAR ls_button.
    ls_button-function  = 'RELEASE'.
    ls_button-icon      = icon_release.
    ls_button-quickinfo = TEXT-b27.
    ls_button-butn_type = 0.
    ls_button-disabled  = space.
    ...
    ls_button-butn_type = 0.
    ls_button-quickinfo = TEXT-b17.
    ls_button-disabled  = space.
    APPEND ls_button TO e_object->mt_toolbar.

    " abort button
    CLEAR ls_button.
    ls_button-function  = 'ABORT'.
    ls_button-icon      = icon_breakpoint.
    ls_button-butn_type = 0.
    ls_button-quickinfo = TEXT-b20.
    ls_button-disabled  = space.
    APPEND ls_button TO e_object->mt_toolbar.
  ENDMETHOD.
  ...
  METHOD on_handle_job_user_command.
    ...
    DATA lt_lvc_t_roid    TYPE lvc_t_roid.
    DATA l_ress_guid      TYPE /cadaxo/sqlcress-ress_guid.
    DATA l_rc             TYPE c LENGTH 1.
    DATA l_cnt            TYPE i.
    DATA l_total          TYPE i.
    DATA l_perc           TYPE p LENGTH 8 DECIMALS 2.
    DATA l_error          TYPE c LENGTH 1.
    DATA l_status         TYPE tbtco-status.
    "   ls_tbtco        TYPE tbtco.

    DATA lt_new_step_list TYPE TABLE OF tbtcstep.

    FIELD-SYMBOLS <l_cont_grid_result> TYPE /cadaxo/sqlcclguicontainer.
    FIELD-SYMBOLS <l_lvc_s_roid>       TYPE lvc_s_roid.
    FIELD-SYMBOLS <l_jobs>             LIKE LINE OF gt_jobs.
    ...
    CASE e_ucomm.
      WHEN 'SELDATE'.
        CALL FUNCTION '/CADAXO/SQLCGETDATEFROMTO'
          CHANGING   c_timestamp_from = g_sel_job_timestamp_from
                     c_timestamp_to   = g_sel_job_timestamp_to
          EXCEPTIONS cancel_by_user   = 1
                     OTHERS           = 2.
        CASE sy-subrc.
          WHEN 0.
            show_jobmonitor( ).
      ...
      WHEN 'RELEASE'.
        ...
        ASSIGN gcont_grid_result_t[ 1 ] TO <l_cont_grid_result>.
        IF sy-subrc = 0.

          <l_cont_grid_result>-gui_alv_grid->get_selected_rows( IMPORTING et_row_no = lt_lvc_t_roid ).

          LOOP AT lt_lvc_t_roid ASSIGNING <l_lvc_s_roid>.
            ASSIGN gt_jobs[ <l_lvc_s_roid>-row_id ] TO <l_jobs>.
            IF sy-subrc <> 0.
              CONTINUE.
            ENDIF.
            ...
            SELECT SINGLE status FROM tbtco
              INTO l_status
              WHERE jobname  = <l_jobs>-jobname
                AND jobcount = <l_jobs>-jobcount.
            IF NOT ( sy-subrc = 0 AND l_status = 'P' ).
              CONTINUE.
            ENDIF.

            CALL FUNCTION 'BP_JOB_MODIFY'
              EXPORTING  jobname       = <l_jobs>-jobname
                         jobcount      = <l_jobs>-jobcount
                         dialog        = 'Y'
                         opcode        = 17 " Release job
              TABLES     new_steplist  = lt_new_step_list
          ...
          <l_cont_grid_result>-gui_alv_grid->refresh_table_display( i_soft_refresh = 'X' ).
              ...
              LOOP AT lt_lvc_t_roid ASSIGNING <l_lvc_s_roid>.
                ...
                IF sy-subrc = 0.
                  ...
                  IF sy-subrc = 0.
                    ...
                    " TODO: variable is assigned but never used (ABAP cleaner)
                    SELECT SINGLE @abap_true FROM tbtco
                      INTO @DATA(l_true)
                      WHERE jobname  = @<l_jobs>-jobname
                    ...
                    IF sy-subrc = 0.
                      ...
                      CALL FUNCTION 'BP_JOB_DELETE'
                        EXPORTING  jobcount                 = <l_jobs>-jobcount
                                   jobname                  = <l_jobs>-jobname
                                   forcedmode               = 'X'
                                   commitmode               = ' '
                        EXCEPTIONS cant_delete_event_entry  = 1
                                   cant_delete_job          = 2
          ...
          IF lt_lvc_t_roid[] IS NOT INITIAL.
            LOOP AT lt_lvc_t_roid ASSIGNING <l_lvc_s_roid>.
              ...
              CALL FUNCTION 'BP_JOB_ABORT'
                EXPORTING  jobcount                   = <l_jobs>-jobcount
                           jobname                    = <l_jobs>-jobname
                EXCEPTIONS checking_of_job_has_failed = 1
  ...
  METHOD on_handle_result_menu_button.
    ...
    DATA l_progname      TYPE c LENGTH 32 VALUE '==============================CP'.
    DATA l_length        TYPE i.

    DATA l_grid_name     TYPE string.
    DATA l_grid_name_i   TYPE i.
    DATA l_result_nr     TYPE n LENGTH 2.
    ...
    DATA l_compare_able  TYPE c LENGTH 1.
    ...
    " TODO: variable is assigned but never used (ABAP cleaner)
    FIELD-SYMBOLS <lr_dref>        TYPE REF TO data.
    FIELD-SYMBOLS <lt_fcat_source> TYPE lvc_t_fcat.
    FIELD-SYMBOLS <ls_fcat_source> TYPE lvc_s_fcat.
    FIELD-SYMBOLS <lt_fcat_target> TYPE lvc_t_fcat.
    " TODO: variable is assigned but never used (ABAP cleaner)
    FIELD-SYMBOLS <ls_fcat_target> TYPE lvc_s_fcat.
...
*          text  = text-f14 ).
        ...
        lr_menu->add_submenu( menu = lr_menu_export
                              text = TEXT-f06 ).

        lr_menu->add_separator( ).

        lr_menu->add_function( fcode = cl_gui_alv_grid=>mc_fc_graph
                               text  = TEXT-f13 ).
      ...
      WHEN 'COMPARE_RESULT'.
        ...
        ASSIGN gt_lvc_t_fcat[ l_grid_name_i ] TO <lt_fcat_source>.
        IF sy-subrc = 0.
          ...
          LOOP AT dref_result_tab_t ASSIGNING <lr_dref>.
            ...
            ASSIGN gt_lvc_t_fcat[ sy-tabix ] TO <lt_fcat_target>.
            IF sy-subrc <> 0.
              CONTINUE.
            ENDIF.
            ...
            l_lines_source = LINES( <lt_fcat_source> ).
            l_lines_target = LINES( <lt_fcat_target> ).
...
*            ELSE.                                                "$001
            LOOP AT <lt_fcat_source> ASSIGNING <ls_fcat_source> WHERE key <> space. "$001
              ASSIGN <lt_fcat_target>[ sy-tabix ] TO <ls_fcat_target>.
              IF sy-subrc <> 0.
                l_compare_able = abap_false.
                EXIT.
              ELSE.

                IF    <ls_fcat_source>-row_pos  <> <ls_fcat_source>-row_pos
                   OR <ls_fcat_source>-col_pos  <> <ls_fcat_source>-col_pos
                   OR <ls_fcat_source>-datatype <> <ls_fcat_source>-datatype
                   OR <ls_fcat_source>-inttype  <> <ls_fcat_source>-inttype
                   OR <ls_fcat_source>-intlen   <> <ls_fcat_source>-intlen.
                  l_compare_able = abap_false.
                  EXIT.
                ENDIF.
              ENDIF.
...
*            ENDIF.   "$001
            ...
            IF l_compare_able = abap_true.
              ...
              e_object->add_function( fcode = l_fcode
                                      text  = l_text ).
      ...
      WHEN 'EXPORT'.
        ...
        ASSIGN lt_table[ id  = 'I'
                         key = TEXT-032 ] TO <ls_table>.
        " to be implemented

        e_object->add_function( fcode = cl_gui_alv_grid=>mc_fc_call_xxl
                                text  = 'Spreadsheet' ).

        e_object->add_function( fcode = cl_gui_alv_grid=>mc_fc_word_processor
                                text  = 'Word Processing' ).

        e_object->add_function( fcode = cl_gui_alv_grid=>mc_fc_pc_file
                                text  = 'Local File' ).

        e_object->add_function( fcode = cl_gui_alv_grid=>mc_fc_send
                                text  = 'Send' ).

        e_object->add_function( fcode = cl_gui_alv_grid=>mc_fc_to_office
                                text  = 'Office' ).

        e_object->add_function( fcode = cl_gui_alv_grid=>mc_fc_html
                                text  = 'HTML File' ).
  ...
  METHOD on_handle_result_toolbar.
    ...
    l_grid_name_i = get_current_grid_number( ).
    ...
    IF l_icon IS INITIAL.
      l_icon = icon_fix_column.
      l_checked = abap_false.
    ENDIF.
    ls_button-function  = 'KEYFIX'.
    ls_button-icon      = l_icon.
    ls_button-butn_type = 0.
    ls_button-quickinfo = TEXT-q31.
    ls_button-disabled  = space.
    ls_button-checked   = l_checked.
    APPEND ls_button TO e_object->mt_toolbar.

    " show result table on full screen
    CLEAR ls_button.

    ls_button-function  = 'RESFULLDISP'.
    ls_button-icon      = icon_view_maximize.
    ls_button-quickinfo = TEXT-q27.
    ls_button-butn_type = 0.
    ls_button-disabled  = space.
...
*  ENDIF.
    ...
    ls_button-function  = 'DUMMY_LIST_NR'.
    ls_button-quickinfo = TEXT-q34.
    ls_button-butn_type = 0.
    ls_button-disabled  = abap_true.
    INSERT ls_button INTO e_object->mt_toolbar INDEX 1.
    ...
    IF lr_sql_parse->g_saved_list IS INITIAL.
      ...
      CALL BADI lr_badi->add_result_toolbar
        CHANGING ct_buttons = lt_buttons.
...
*     field-symbols: <ls_result_line> type any.
  ...
  METHOD on_handle_savedlists_toolbar.
    ...
    " refresh button
    CLEAR ls_button.
    ls_button-function  = 'RENAME'.
    ls_button-icon      = icon_rename.
    ls_button-quickinfo = TEXT-b30.
    ls_button-butn_type = 0.
    ls_button-disabled  = space.
    APPEND ls_button TO e_object->mt_toolbar.
    ...
    ls_button-icon      = icon_delete.
    ls_button-butn_type = 0.
    ls_button-quickinfo = TEXT-b24.
    ls_button-disabled  = space.
    APPEND ls_button TO e_object->mt_toolbar.

    " import button
    CLEAR ls_button.
    ls_button-function  = 'IMPORT_SQLX'.
    ls_button-icon      = icon_import.
    ls_button-butn_type = 0.
    ls_button-disabled  = space.
    ls_button-quickinfo = TEXT-b29.
    APPEND ls_button TO e_object->mt_toolbar.

    " export button
    CLEAR ls_button.
    ls_button-function  = 'EXPORT_SQLX'.
    ls_button-icon      = icon_export.
    ls_button-butn_type = 0.
    ls_button-disabled  = space.
    ls_button-quickinfo = TEXT-b28.
    APPEND ls_button TO e_object->mt_toolbar.
...
*                    butn_type = 2 "-Cockpit-420
  ...
  METHOD on_handle_savedlists_usrcommnd.
    ...
    FIELD-SYMBOLS <l_cont_grid_result> TYPE /cadaxo/sqlcclguicontainer.
    FIELD-SYMBOLS <l_lvc_s_roid>       TYPE lvc_s_roid.
    FIELD-SYMBOLS <ls_saved_lists>     LIKE LINE OF me->gt_saved_lists.
      ...
      WHEN 'SELECT'.
        ...
        IF sy-subrc = 0.
          ...
          IF lt_lvc_t_roid[] IS NOT INITIAL.
            CLEAR lt_ress_guid.
            LOOP AT lt_lvc_t_roid ASSIGNING <l_lvc_s_roid>.
              ASSIGN me->gt_saved_lists[ <l_lvc_s_roid>-row_id ] TO <ls_saved_lists>.
              IF sy-subrc = 0.
          ...
          ELSE.
            ...
            IF lines( lt_cells ) > 0.
              LOOP AT lt_cells ASSIGNING FIELD-SYMBOL(<ls_cell>).
                ASSIGN me->gt_saved_lists[ <ls_cell>-row_id-index ] TO <ls_saved_lists>.
                IF sy-subrc = 0.
                  APPEND <ls_saved_lists>-ress_guid TO lt_ress_guid.
                ENDIF.
              ENDLOOP.
          ...
          IF lines( lt_ress_guid ) > 0.
            get_saved_results( lt_ress_guid ).
          ENDIF.
        ...
        IF sy-subrc = 0.
          ...
          LOOP AT lt_lvc_t_roid ASSIGNING <l_lvc_s_roid>.
            ...
            CALL FUNCTION '/CADAXO/SQLCLISTS_REN_SAV_LIST'
              IMPORTING e_rc          = l_rc
              CHANGING  c_description = l_description.

            IF l_description <> <ls_saved_lists>-description AND l_rc = 'O'.
              /cadaxo/cl_sqlc_cockpit_lists=>rename_list( i_list_guid   = <ls_saved_lists>-list_guid
                                                          i_jobcount    = <ls_saved_lists>-jobcount
...
*      WHEN c_saved_list_share. "cockpit-401    "-cockpit-420
      ...
      WHEN c_saved_list_share_me.
        ...
        share_saved_list( iv_receiver = CONV /cadaxo/sqlcapi_receiver( sy-uname )
                          iv_text     = TEXT-012 ).
  ...
  METHOD on_job_alv_click.
    ...
    ASSIGN gt_jobs[ es_row_no-row_id ] TO <ls_jobs>.
    IF sy-subrc = 0.
  ...
  METHOD on_job_alv_hotspot_click.
    ...
    IF sy-subrc = 0.
      ...
      PERFORM show_job_sm37b IN PROGRAM saplbtch IF FOUND TABLES lt_tbtcjob
  ...
  METHOD on_log_alv_context_menu.
    ...
    TRY.
        ...
        gui_alv_grid->get_current_cell( IMPORTING es_row_id = row
                                                  es_col_id = col ).
  ...
  METHOD on_log_alv_double_click.
    ...
    DATA l_to_line       TYPE i.
    DATA l_to_pos        TYPE i.
    DATA lt_string_block TYPE TABLE OF string.
    DATA l_sql_string    TYPE string.
    ...
    ASSIGN gt_history_log[ e_row-index ] TO <l_history_log>.
    IF sy-subrc <> 0.
      RETURN.
    ENDIF.
  ...
  METHOD on_log_alv_drag.
    ...
    ASSIGN gt_history_log[ e_row-index ] TO FIELD-SYMBOL(<ls_history_log>).
  ...
  METHOD on_log_alv_toolbar.
    ...
    " append separator
    ls_button-butn_type = lc_separator.
    APPEND ls_button TO e_object->mt_toolbar.
    ...
    " delete my history
    CLEAR ls_button.
    ls_button-function  = 'DELHIST'.
    ls_button-icon      = icon_delete.
    ls_button-quickinfo = TEXT-b25.
    ls_button-butn_type = 0.
    ls_button-disabled  = space.
    APPEND ls_button TO e_object->mt_toolbar.
  ...
  METHOD on_log_alv_user_command.
    ...
    CASE e_ucomm.
      WHEN 'SELDATE'.
        CALL FUNCTION '/CADAXO/SQLCGETDATEFROMTO'
          CHANGING   c_timestamp_from = g_sel_hist_timestamp_from
      ...
      WHEN 'DELHIST'.
        /cadaxo/cl_sqlc_user_hist_log=>delete_log( ).
      ...
      WHEN c_cmd_show_full_value.
        handle_command_show_full_value( i_log = abap_true ).
  ...
  METHOD on_result_toolbar_dropdown.
    ...
    IF me->g_result_layout-no_toolbar = abap_true.
      lr_alv_options->add_function( fcode    = c_cmd_result_toolbar_show
                                    disabled = lv_no_toolbar
                                    text     = TEXT-m04 ).
    ELSE.
      lr_alv_options->add_function( fcode    = c_cmd_result_toolbar_hide
                                    disabled = lv_no_toolbar
                                    text     = TEXT-m03 ).
    ENDIF.

    IF me->g_user_settings-show_footer = abap_true.
      lr_alv_options->add_function( fcode    = c_cmd_result_footer_hide
                                    disabled = lv_no_toolbar
                                    text     = TEXT-m05 ).
    ELSE.
      lr_alv_options->add_function( fcode    = c_cmd_result_footer_show
                                    disabled = lv_no_toolbar
                                    text     = TEXT-m06 ).
    ENDIF.
    ...
    lr_alv_options_window->add_function( fcode    = 'WINDOW_VERTICAL'
                                         disabled = l_disabled
                                         text     = TEXT-m08 ).
    IF lv_no_toolbar = abap_false.
    ...
    ELSE.
      l_disabled = abap_false.
    ENDIF.
    lr_alv_options_window->add_function( fcode    = 'WINDOW_HORIZONTAL'
                                         disabled = l_disabled
                                         text     = TEXT-m09 ).
    IF lv_no_toolbar = abap_false.
    ...
    ELSE.
      l_disabled = abap_false.
    ENDIF.
    lr_alv_options_window->add_function( fcode    = 'WINDOW_MATRIX'
                                         disabled = l_disabled
                                         text     = TEXT-m10 ).
    ...
    ELSE.
      l_disabled = abap_false.
    ENDIF.
    lr_alv_options_window->add_function( fcode    = 'WINDOW_TAB'
                                         disabled = l_disabled
                                         text     = TEXT-m11 ).

    lr_alv_options->add_submenu( menu     = lr_alv_options_window
                                 disabled = lv_no_toolbar
                                 text     = TEXT-m07 ).

    gc_result_toolbar->track_context_menu( context_menu = lr_alv_options
                                           posx         = posx
                                           posy         = posy ).
  ENDMETHOD.
  ...
  METHOD on_result_toolbar_funcsel.
    ...
    IF fcode = c_cmd_show_saved_lists OR fcode = c_cmd_jobmonitor OR fcode = c_cmd_home OR fcode = c_cmd_show_result_table OR fcode = c_cmd_show_log.
      set_result_toolbar_active( i_fcode = fcode ).
    ENDIF.
    ...
    CASE fcode.
      WHEN c_cmd_result_toolbar_hide OR c_cmd_result_toolbar_show.
        ...
        IF me->g_user_settings-result_buttons <> abap_true.
          ms_user_settings_xml-result_buttons = abap_true.
          g_user_settings-result_buttons      =
          ms_user_settings_xml-result_buttons.
          g_result_layout-no_toolbar          = abap_false.
        ELSE.
          ms_user_settings_xml-result_buttons = abap_false.
          g_user_settings-result_buttons      =
          ms_user_settings_xml-result_buttons.
          g_result_layout-no_toolbar          = abap_true.
        ENDIF.
        ...
        LOOP AT gcont_grid_result_t ASSIGNING <ls_cont_grid_result>.
          ...
          ls_layout-no_toolbar = g_result_layout-no_toolbar.
          ...
          <ls_cont_grid_result>-gui_alv_grid->set_frontend_layout( is_layout = ls_layout ).
        ENDLOOP.

      WHEN 'WINDOW_VERTICAL'.
        ms_user_settings_xml-reswindoworientation = cs_windowresolution-vertical.
        g_user_settings-result_window_horizontal = space.
        g_user_settings-result_window_vertical   = abap_true.
        g_user_settings-result_window_matrix     = space.
        g_user_settings-result_window_tab        = space.
        show_result( ).
      WHEN 'WINDOW_HORIZONTAL'.
        ms_user_settings_xml-reswindoworientation = cs_windowresolution-horizontal.
        g_user_settings-result_window_horizontal = abap_true.
        g_user_settings-result_window_vertical   = space.
        g_user_settings-result_window_matrix     = space.
        g_user_settings-result_window_tab        = space.
        show_result( ).
      WHEN 'WINDOW_MATRIX'.
      ...
      WHEN c_cmd_result_footer_hide.
        ms_user_settings_xml-show_footer = abap_false.
        g_user_settings-show_footer      = abap_false.
        show_result( ).
      WHEN c_cmd_result_footer_show.
        ms_user_settings_xml-show_footer = abap_true.
        g_user_settings-show_footer      = abap_true.
        show_result( ).
  ...
  METHOD on_saved_list_menu_click.
    ...
    lr_menu->add_function( fcode             = c_saved_list_share_oth
                           text              = TEXT-b48
                           icon              = icon_workflow_external_event
                           insert_at_the_top = abap_true
                           checked           = abap_true ). " Cockpit-420 KA
    lr_menu->add_function( fcode = c_saved_list_share_me
                           text  = TEXT-b44
  ...
  METHOD on_saved_list_select_line.
    ...
    ASSIGN me->gt_saved_lists[ es_row_no-row_id ] TO <ls_saved_lists>.
    IF sy-subrc = 0.
      CLEAR lt_ress_guid.
      APPEND <ls_saved_lists>-ress_guid TO lt_ress_guid.
      get_saved_results( lt_ress_guid ).
  ...
  METHOD pai_0100.
    ...
    symbols_controller->check_changed_data( ).
...
*      me->usr_action_share( ).                      "COCKPIT-233
      ...
      WHEN 'LEAVE'.       " Leave SQL Cockpit :-(
        usr_action_leave_sql_cockpit( ).
      WHEN 'CANCEL'.      " Clear SQL Area
        lt_table = get_sql_area_lt_code( ).
        IF lt_table[] IS NOT INITIAL.
          CALL FUNCTION 'POPUP_TO_CONFIRM'
            EXPORTING text_question = TEXT-q26
            IMPORTING answer        = lv_rc.
          IF lv_rc = '1'.
            usr_action_clear_sql_area( ).
          ENDIF.
        ENDIF.
  ...
  METHOD pai_0800.
    ...
    CASE i_ok_code.
      WHEN 'OK'.
        ...
        /cadaxo/cl_sqlc_cockpit_lists=>save_list( i_cl_sql_parse    = gt_cl_sql_parse
                                                  it_result_details = gt_result_details
                                                  it_grid_results   = gcont_grid_result_t
      ...
      WHEN 'SETTOREAD'.
        ...
        ELSE.
          LOOP AT lt_rows ASSIGNING FIELD-SYMBOL(<ls_rows>).
            ASSIGN gt_queue[ <ls_rows>-index ] TO FIELD-SYMBOL(<ls_queue>).
            IF sy-subrc = 0.
        ...
        ELSE.
          ...
          IF lv_answer = 1.
            ...
            LOOP AT lt_rows ASSIGNING <ls_rows>.
              ASSIGN gt_queue[ <ls_rows>-index ] TO <ls_queue>.
              IF sy-subrc = 0.
...
*<VERSIONNR>1.0</VERSIONNR>
    ...
    CALL FUNCTION 'CONVERSION_EXIT_ISOLA_OUTPUT'
      EXPORTING input  = sy-langu
      IMPORTING output = l_langu.
  ...
  METHOD pbo_2000.
    ...
    IF gcont_alv_template IS INITIAL.
      ...
      gcont_alv_template = NEW #( container_name = 'GCONT_ALV_TEMPLATE' ).

      gc_alv_template_2000 = NEW #( i_parent = gcont_alv_template ).

      " create field catalog
      CALL FUNCTION 'LVC_FIELDCATALOG_MERGE'
        EXPORTING  i_structure_name       = '/CADAXO/SQLCTEMP_ALV'
      ...
      IF sy-subrc = 0.
        ...
        gc_alv_template_2000->set_table_for_first_display( EXPORTING  i_bypassing_buffer = abap_true
                                                                      is_layout          = l_lvc_s_layo
                                                           CHANGING   it_outtab          = gt_templates
                                                                      it_fieldcatalog    = lt_fieldcat
                                                           EXCEPTIONS OTHERS             = 4 ).
        IF sy-subrc = 0.
      ...
      ELSE.
        gc_alv_template_2000->refresh_table_display( ).
      ENDIF.
    ENDIF.
...
*    SORT gt_queue BY date time DESCENDING.                                                           "COCKPIT-294
    ...
    IF gcont_alv_queue IS INITIAL.
      ...
      " create field catalog
      CALL FUNCTION 'LVC_FIELDCATALOG_MERGE'
        EXPORTING  i_structure_name       = '/CADAXO/SQLCAPI_QUEUE'
                   i_client_never_display = abap_true
                   i_bypassing_buffer     = abap_true
        CHANGING   ct_fieldcat            = fieldcats
        EXCEPTIONS OTHERS                 = 1.
      IF sy-subrc = 0.

        LOOP AT fieldcats ASSIGNING FIELD-SYMBOL(<fieldcat>).

          IF NOT (    <fieldcat>-fieldname CS 'POSITION_TYP_I' " Icons only
                   OR <fieldcat>-fieldname  = 'DESCRIPTION'
                   OR <fieldcat>-fieldname  = 'DATE'
                   OR <fieldcat>-fieldname  = 'TIME'
                   OR <fieldcat>-fieldname  = 'SENDER' ).
            CONTINUE.
          ENDIF.
...
*                                             cwidth_opt = abap_true "COCKPIT-465
        ...
        gc_alv_queue_3000->set_table_for_first_display( EXPORTING  i_bypassing_buffer = abap_true
                                                                   is_layout          = alv_layout
                                                        CHANGING   it_outtab          = gt_queue
      ...
      ELSE.
        gc_alv_queue_3000->refresh_table_display( is_stable      = VALUE lvc_s_stbl( row = abap_true
                                                                                     col = abap_true  )
  ...
  METHOD populate_saved_list.
    SELECT SINGLE * FROM /cadaxo/sqlcsres
      INTO @DATA(ls_sqlcsres)
      WHERE list_guid = @iv_list_guid.
    IF sy-subrc = 0.

      SELECT SINGLE * FROM /cadaxo/sqlcress
  ...
  METHOD prepare_result_table.
    ...
    DATA lv_guid22                TYPE c LENGTH 22.
    DATA lv_component             TYPE string.
    " TODO: variable is assigned but never used (ABAP cleaner)
    DATA l_string                 TYPE string.
    ...
    FIELD-SYMBOLS <ls_lvc_t_fcat>   TYPE LINE OF lvc_t_fcat.

    cl_abap_gzip=>decompress_text( EXPORTING gzip_in  = is_sqlcress-rawdata
                                   IMPORTING text_out = l_xml ).

    IF is_sqlcress-editor_sqlstring IS NOT INITIAL.

      IMPORT code TO lt_code FROM DATA BUFFER is_sqlcress-editor_sqlstring.

      set_sql_area( i_codelines_t = lt_code[]  ).

    ENDIF.

    " SQL-79
    replace_old_runtime_structure( CHANGING xml = l_xml ).
    ...
    LOOP AT lt_sqlcresultsave INTO ls_sqlcresultsave.
      ...
      lr_sqlc_cl_cockpit_parse = NEW #( ).

      lr_sqlc_cl_cockpit_parse->gt_result_ddfields       = ls_sqlcresultsave-parse-result_ddfields.
      lr_sqlc_cl_cockpit_parse->column_syntax            = ls_sqlcresultsave-parse-column_syntax.
      lr_sqlc_cl_cockpit_parse->source_syntax            = ls_sqlcresultsave-parse-source_syntax.
      lr_sqlc_cl_cockpit_parse->where_syntax             = ls_sqlcresultsave-parse-where_syntax.
      lr_sqlc_cl_cockpit_parse->group_syntax             = ls_sqlcresultsave-parse-group_syntax.
      lr_sqlc_cl_cockpit_parse->having_syntax            = ls_sqlcresultsave-parse-having_syntax.
      lr_sqlc_cl_cockpit_parse->order_syntax             = ls_sqlcresultsave-parse-order_syntax.
      lr_sqlc_cl_cockpit_parse->dbhint_syntax            = ls_sqlcresultsave-parse-dbhint_syntax.
      lr_sqlc_cl_cockpit_parse->result_source_t          = ls_sqlcresultsave-parse-result_source.
      lr_sqlc_cl_cockpit_parse->sql_syntax               = ls_sqlcresultsave-parse-sql_syntax.
      lr_sqlc_cl_cockpit_parse->connection_syntax        = ls_sqlcresultsave-parse-connection_syntax.
      lr_sqlc_cl_cockpit_parse->g_up_to_x_rows           = ls_sqlcresultsave-parse-up_to_x_rows.
      lr_sqlc_cl_cockpit_parse->subquery                 = ls_sqlcresultsave-parse-subquery.
      lr_sqlc_cl_cockpit_parse->g_bypassing_buffer       = ls_sqlcresultsave-parse-bypassing_buffer.
      lr_sqlc_cl_cockpit_parse->sql_syntax_without_where = ls_sqlcresultsave-parse-sql_syntax_without_where.
      lr_sqlc_cl_cockpit_parse->g_select_single          = ls_sqlcresultsave-parse-select_single.
      lr_sqlc_cl_cockpit_parse->gs_client_handling-client_specified = ls_sqlcresultsave-parse-client_specified.
      lr_sqlc_cl_cockpit_parse->gs_client_handling-using_client     = ls_sqlcresultsave-parse-using_client.
      lt_saved_lvc_t_fcat = ls_sqlcresultsave-parse-result_fieldcatalog.
      lt_saved_lvc_t_sort = ls_sqlcresultsave-parse-result_sort.
      lt_saved_lvc_t_filt = ls_sqlcresultsave-parse-result_filt.
      ls_saved_lvc_s_layo = ls_sqlcresultsave-parse-result_layo. "#4170
      lt_components_domval = ls_sqlcresultsave-parse-components_domval. " COCKPIT-468
      lt_comp = ls_sqlcresultsave-parse-comp.              " COCKPIT-468
      lt_domval = ls_sqlcresultsave-parse-domval.            " COCKPIT-468
      lr_sqlc_cl_cockpit_parse->g_saved_list = abap_true.
      ...
      LOOP AT ls_sqlcresultsave-parse-result_ddfields INTO l_result_ddfields.
        ...
        IF l_result_ddfields-aggr = 'SUM('.
          CASE l_result_ddfields-datatype.
            WHEN 'CURR' OR 'DEC' OR 'QUAN'.
              ...
              l_sql_abap_componentdescr-type ?= cl_abap_elemdescr=>get_p( p_length   = 16
        ...
        ELSE.
          ...
          IF l_result_ddfields-datatype = 'UTCL'.
            CALL METHOD cl_abap_elemdescr=>('GET_UTCLONG')
              RECEIVING p_result = l_sql_abap_componentdescr-type.
          ELSE.
            " COCKPIT-468
            lr_data = /cadaxo/cl_sqlc_cockpit_assist=>create_data_reference(
                          ...
                          iv_tabix             = tabix
                          iv_domaintext        = g_user_settings-domaintext
        ...
        ELSEIF l_result_ddfields-tabname <> space.        " CDX-RT138
          l_sql_abap_componentdescr-name = l_result_ddfields-tabname. " CDX-RT138
        ELSE.                                             " CDX-RT138
          l_sql_abap_componentdescr-name = l_result_ddfields-fieldname. " CDX-RT138
        ENDIF.                                            " CDX-RT138

        IF line_exists( lr_sqlc_cl_cockpit_parse->result_component_t[ name = l_sql_abap_componentdescr-name ] ) OR strlen(
            l_sql_abap_componentdescr-name ) > 30.
          IF l_result_ddfields-map_fieldname IS INITIAL.  " RT377
            DO.
              TRY.
                  CALL METHOD ('CL_SYSTEM_UUID')=>('CREATE_UUID_C22_STATIC')
                    RECEIVING uuid = lv_guid22.
                CATCH cx_root.
                  CALL FUNCTION 'GUID_CREATE'
                    IMPORTING ev_guid_22 = lv_guid22.
              ENDTRY.
              lv_guid22 = to_upper( lv_guid22 ).
            ...
            " RT337 BEGIN
            ASSIGN lt_saved_lvc_t_fcat[ fieldname = l_sql_abap_componentdescr-name ] TO <ls_lvc_t_fcat>. " RT377
            IF sy-subrc = 0.                              " RT377
        ...
        IF me->g_user_settings-domaintext = abap_true.
      ...
      IF lt_saved_lvc_t_fcat[] IS INITIAL.
        lt_lvc_t_fcat = lr_sqlc_cl_cockpit_parse->create_alv_field_catalog( i_user_settings   = g_user_settings
                                                                            i_dragdrop_handle = dragdrop_handle ).
      ELSE.
      ...
      lr_sqlc_cl_cockpit_parse->create_result_structures( i_mode = 'S' ). " S = Saved Lists

      ASSIGN lr_sqlc_cl_cockpit_parse->result_table->* TO <lt_result_table>.

      IF lt_saved_lvc_t_sort IS NOT INITIAL.
        lt_saved_lvc_t_sort = /cadaxo/cl_sqlc_cockpit_main=>match_saved_sort(
                                  it_sort         = lt_saved_lvc_t_sort
                                  i_tabname       = l_result_ddfields-tabname
                                  it_result_table = <lt_result_table> ).
      ENDIF.

      IF lt_saved_lvc_t_filt IS NOT INITIAL.
        lt_saved_lvc_t_filt = /cadaxo/cl_sqlc_cockpit_main=>match_saved_filter(
                                  it_filter       = lt_saved_lvc_t_filt
                                  i_tabname       = l_result_ddfields-tabname
                                  it_result_table = <lt_result_table> ).
      ...
      IF lt_lvc_t_fcat IS NOT INITIAL.
        lt_lvc_t_fcat = /cadaxo/cl_sqlc_cockpit_main=>match_saved_fieldcat_orig(
                            it_fcat         = lt_lvc_t_fcat
                            i_tabname       = l_result_ddfields-tabname
                            it_result_table = <lt_result_table> ).
      ENDIF.
      ...
      IF lt_lvc_t_fcat IS NOT INITIAL.
        LOOP AT ls_sqlcresultsave-parse-result_ddfields INTO l_result_ddfields.
          ...
          ASSIGN lt_lvc_t_fcat[ fieldname = l_result_ddfields-map_fieldname ] TO <ls_lvc_t_fcat>.
          IF sy-subrc <> 0.
            ASSIGN lt_lvc_t_fcat[ fieldname = l_result_ddfields-fieldname ] TO <ls_lvc_t_fcat>.
            IF sy-subrc <> 0.
              ASSIGN lt_lvc_t_fcat[ fieldname = lv_component ] TO <ls_lvc_t_fcat>.
            ENDIF.
          ENDIF.

          IF <ls_lvc_t_fcat> IS ASSIGNED.

            /cadaxo/cl_sqlc_cockpit_assist=>map_ddfields_descr_to_fcat( EXPORTING is_dfies         = l_result_ddfields
                                                                                  is_user_settings = g_user_settings
                                                                        CHANGING  cs_fcat          = <ls_lvc_t_fcat> ).
...
*        CATCH cx_sy_import_mismatch_error.
      ...
      ls_result_details = ls_sqlcresultsave-main-result_details.
  ...
  METHOD save_clipboard.
    ...
    DATA l_string        TYPE string.
    DATA l_stringx       TYPE xstring.
    DATA l_sql_usrpref   TYPE /cadaxo/sqlcusrp.
    DATA lr_convout      TYPE REF TO cl_abap_conv_out_ce.

    DATA l_text_modified TYPE i.

    FIELD-SYMBOLS <l_text_line> TYPE any.

    gc_clipboard_textedit->get_textmodified_status( IMPORTING  status                 = l_text_modified
                                                    EXCEPTIONS error_cntl_call_method = 1
                                                               OTHERS                 = 2 ).

    IF NOT ( sy-subrc = 0 AND l_text_modified = 1 ).
      RETURN.
    ENDIF.

    " get the text as stream
    gc_clipboard_textedit->get_text_as_stream( IMPORTING  text                   = lt_text
                                               EXCEPTIONS error_dp               = 1
                                                          error_cntl_call_method = 2
                                                          OTHERS                 = 3 ).
    ...
    IF sy-subrc = 0 AND me->gt_clipboard[] <> lt_text[].
      ...
      gc_clipboard_textedit->set_textmodified_status( status = 0 ).
  ...
  METHOD save_hold_lists.
    ...
    LOOP AT gt_cl_sql_parse ASSIGNING FIELD-SYMBOL(<lr_cl_sql_parse>).
      ...
      ASSIGN dref_result_tab_t[ l_index ] TO FIELD-SYMBOL(<ls_result_tab>).
      IF sy-subrc = 0.
        APPEND <ls_result_tab> TO gt_result_tab_hold.
      ENDIF.
      ASSIGN gt_lvc_t_fcat[ l_index ] TO FIELD-SYMBOL(<ls_lvc_s_fcat>).
      IF sy-subrc = 0.
        APPEND <ls_lvc_s_fcat> TO gt_lvc_t_fcat_hold.
      ENDIF.
      ASSIGN gt_result_details[ l_index ] TO FIELD-SYMBOL(<ls_result_details>).
      IF sy-subrc = 0.
  ...
  METHOD select_jobdata.
    ...
    LOOP AT lt_sql_sres ASSIGNING FIELD-SYMBOL(<l_sqlcsres>).
      ...
      l_sqlcjobsalv-uname         = <l_sqlcsres>-uname.
      l_sqlcjobsalv-jobname       = <l_sqlcsres>-jobname.
      l_sqlcjobsalv-jobcount      = <l_sqlcsres>-jobcount.
      l_sqlcjobsalv-list_guid     = <l_sqlcsres>-list_guid.
      l_sqlcjobsalv-ress_guid     = <l_sqlcsres>-ress_guid.
      l_sqlcjobsalv-nr_of_selects = <l_sqlcsres>-nr_of_selects. " CDX130-004
      ...
      IF sy-subrc = 0.
        ...
        l_sqlcjobsalv-end_date = ls_tbtco-enddate.     " CDX130-002
        l_sqlcjobsalv-end_time = ls_tbtco-endtime.     " CDX130-002
        ...
        CASE ls_tbtco-status.
          WHEN 'F'. " Finished
            l_sqlcjobsalv-jobstatus = TEXT-stf.
            ls_lvc_s_scol-fname = 'JOBSTATUS'.
            ls_lvc_s_scol-color-col = '5'.
            APPEND ls_lvc_s_scol TO l_sqlcjobsalv-ct_col.
          WHEN 'P'. " Scheduled
            CLEAR: l_sqlcjobsalv-end_date,
                   l_sqlcjobsalv-end_time. "# 4660 - 20140919
            l_sqlcjobsalv-jobstatus = TEXT-stp.
          WHEN 'S'. " Released
            CLEAR: l_sqlcjobsalv-end_date,
                   l_sqlcjobsalv-end_time. "# 4660 - 20140919
            l_sqlcjobsalv-jobstatus = TEXT-sts.
          WHEN 'R'. " Running
            CLEAR: l_sqlcjobsalv-end_date,
                   l_sqlcjobsalv-end_time. "# 4660 - 20140919
            l_sqlcjobsalv-jobstatus = TEXT-str.
            ls_lvc_s_scol-fname = 'JOBSTATUS'.
            ls_lvc_s_scol-color-col = '5'.
            APPEND ls_lvc_s_scol TO l_sqlcjobsalv-ct_col.
          WHEN 'A'. " Canceled
            l_sqlcjobsalv-jobstatus = TEXT-stc.
            ls_lvc_s_scol-fname = 'JOBSTATUS'.
      ...
      ELSE.
        l_sqlcjobsalv-jobstatus = TEXT-stu.
        ls_lvc_s_scol-fname = 'JOBSTATUS'.
        ls_lvc_s_scol-color-col = '7'.
        APPEND ls_lvc_s_scol TO l_sqlcjobsalv-ct_col.
      ENDIF.

      IF l_sqlcjobsalv-ress_guid IS NOT INITIAL.
        l_sqlcjobsalv-jobresult = '@3W@'.
    ...
    SORT gt_jobs BY create_date DESCENDING
  ...
  METHOD send_sql_via_mail.
    ...
    " get the sql content
    l_sql_string = get_sql_area( ).
    /cadaxo/cl_sqlc_cockpit_assist=>get_parameter_value( EXPORTING  i_parameter_id      = 'CADAXO_REV_VERSION'
                                                         RECEIVING  r_parameter_value   = l_version
                                                         EXCEPTIONS parameter_not_found = 1 ).
    IF sy-subrc <> 0.
      /cadaxo/cl_sqlc_cockpit_assist=>get_parameter_value( EXPORTING  i_parameter_id      = 'CADAXO_VERSION'
                                                           RECEIVING  r_parameter_value   = l_version
                                                           EXCEPTIONS parameter_not_found = 1 ).
    ENDIF.
    ...
    " call frontend mail
    /cadaxo/cl_sqlc_cockpit_assist=>create_frontend_mail( i_mailto  = 'support@cadaxo.com'
                                                          i_subject = l_subject
                                                          i_body    = l_body ).
...
*                IR_MAIN   =
  ...
  METHOD set_clipboard_alv.
    ...
    CASE g_show_clipboard.
      WHEN 'R'.
        ...
        gc_clipboard_toolbar->set_button_info( fcode     = c_okcode_clipboard
                                               icon      = '@K1@'
                                               quickinfo = TEXT-q03 ).
      ...
      WHEN space.
        ...
        gc_clipboard_toolbar->set_button_visible( visible = abap_true
                                                  fcode   = 'CLEAR_CLIPBOARD' ).

        gc_clipboard_toolbar->set_button_info( fcode     = 'CLEAR_CLIPBOARD'
                                               quickinfo = TEXT-q04 ).
  ...
  METHOD set_result_toolbar_active.
    ...
    IF i_fcode <> 'SHOW_RESULT_OPTIONS'.
      ...
      IF dref_result_tab_t[] IS INITIAL.
        gc_result_toolbar->set_button_state( enabled = ' '
                                             fcode   = c_cmd_show_result_table ).
      ELSE.
        gc_result_toolbar->set_button_state( enabled = 'X'
                                             fcode   = c_cmd_show_result_table ).
      ...
      gc_result_toolbar->set_button_state( checked = ''
                                           fcode   = 'SHOW_LOG' ).
      gc_result_toolbar->set_button_state( checked = ''
                                           fcode   = c_cmd_jobmonitor ).
      gc_result_toolbar->set_button_state( checked = ''
                                           fcode   = c_cmd_show_saved_lists ). " CDX130-030

      " set checked yes
      gc_result_toolbar->set_button_state( checked = 'X'
                                           fcode   = mv_toolbar_result_active ).

      " save list button
      IF me->is_result_filled( ) IS INITIAL OR me->mv_toolbar_result_active <> c_cmd_show_result_table.
        gc_splitter_top_toolbar->set_button_state( enabled = abap_false
                                                   fcode   = 'SAVE_LISTS' ).
      ELSE.
        gc_splitter_top_toolbar->set_button_state( enabled = abap_true
                                                   fcode   = 'SAVE_LISTS' ).
      ENDIF.
  ...
  METHOD share_saved_list.
    ...
    <ls_cont_grid_result>-gui_alv_grid->get_selected_rows( IMPORTING et_row_no = DATA(lt_lvc_roid) ).
    ...
    ELSE.
      ...
      IF sy-subrc = 0.
        ...
        DATA(saved_list_for_sharing) = populate_saved_list(
                                           iv_list_guid         = gt_saved_lists[ <ls_lvc_s_roid>-row_id ]-list_guid
                                           iv_saved_list_shared = gc_saved_list_shared ).
        CALL FUNCTION '/CADAXO/SQLC_SHARE'
          EXPORTING iv_export_type = /cadaxo/cl_sqlc_cockpit_api=>cs_api_types-savedlist
  ...
  METHOD show_html.
    ...
    IF g_is_its IS INITIAL.
      gs_splitter_results->set_grid( rows    = 1
                                     columns = 1 ).
    ENDIF.
    ...
    gcont_html_viewer = gs_splitter_results->get_container( row    = 1
                                                            column = 1 ).
    gcont_html_viewer->set_name( 'CONTAINER' ).

    " create the html control
    gc_html_viewer = NEW #( parent = gcont_html_viewer ).
    ...
    gc_html_viewer->set_registered_events( events = lt_event ).
    ...
    get_content( EXPORTING i_html_id      = l_html_id
                 ...
                 IMPORTING et_content     = lt_cache
                           e_assigend_url = l_doc_url ).

    " local or link home screen
    get_link( EXPORTING i_html_id = l_html_id
              IMPORTING e_url     = l_doc_url
              CHANGING  ct_cache  = lt_cache ).

    l_doc_url_s = l_doc_url.
    param_replace_tags( CHANGING data = l_doc_url_s ).
    l_doc_url = l_doc_url_s.

    IF lt_cache IS NOT INITIAL.
      " load the html data
    ...
    ELSEIF l_doc_url IS NOT INITIAL.
      gc_html_viewer->show_url( url = l_doc_url ).
    ELSE.
  ...
  METHOD show_jobmonitor.
    ...
    l_lvc_s_layo-grid_title = /cadaxo/cl_sqlc_cockpit_assist=>create_alv_date_header(
                                  i_timestamp_from = g_sel_job_timestamp_from
    ...
    IF lt_lfc_t_fcat IS INITIAL.
      ...
      " build field catalog
      CALL FUNCTION 'REUSE_ALV_FIELDCATALOG_MERGE'
        EXPORTING  i_structure_name       = '/CADAXO/SQLCJOBSALV'
        CHANGING   ct_fieldcat            = lt_fieldcat
        EXCEPTIONS inconsistent_interface = 1
                   program_error          = 2
                   OTHERS                 = 3.
...
*            l_lvc_s_fcat-HOTSPOT = 'X'.
    ...
    <l_cont_grid_result>-gui_container = gs_splitter_results->get_container( row    = 1
                                                                             column = 1 ).

    <l_cont_grid_result>-gui_container->set_name( 'CONTAINER' ).

    <l_cont_grid_result>-gui_alv_grid = NEW #( i_parent = <l_cont_grid_result>-gui_container ).

    SET HANDLER me->on_job_alv_click           FOR <l_cont_grid_result>-gui_alv_grid.
    SET HANDLER me->on_handle_job_toolbar      FOR <l_cont_grid_result>-gui_alv_grid.
    SET HANDLER me->on_handle_job_user_command FOR <l_cont_grid_result>-gui_alv_grid.
    SET HANDLER me->on_job_alv_hotspot_click   FOR <l_cont_grid_result>-gui_alv_grid.

    <l_cont_grid_result>-gui_alv_grid->set_table_for_first_display(
      EXPORTING  i_bypassing_buffer            = 'X'
                 is_layout                     = l_lvc_s_layo
                 it_toolbar_excluding          = g_jobmonitor_toolbar_ex
      CHANGING   it_outtab                     = gt_jobs
  ...
  METHOD show_log.
    ...
    " create dragdrop behavior
    IF me->g_abap_editor_type = editor_type-new AND dragdrop_behaviour_log IS INITIAL.
      dragdrop_behaviour_log = NEW #( ).
    ...
    DATA(lt_sql_log) = /cadaxo/cl_sqlc_user_hist_log=>select_log( i_timestamp_from = g_sel_hist_timestamp_from
                                                                  i_timestamp_to   = g_sel_hist_timestamp_to ).

    gt_history_log = /cadaxo/cl_sqlc_user_hist_log=>convert_to_alv( lt_sql_log ).

    SORT gt_history_log BY execute_date DESCENDING
    ...
    DATA(layout) = VALUE lvc_s_layo( zebra      = abap_true
                                     ...
                                     grid_title = /cadaxo/cl_sqlc_cockpit_assist=>create_alv_date_header(
                                                      i_timestamp_from = g_sel_hist_timestamp_from
                                                      i_timestamp_to   = g_sel_hist_timestamp_to ) ).

    DATA select_log_fildcats TYPE lvc_t_fcat.

    IF select_log_fildcats IS INITIAL.

      CALL FUNCTION 'REUSE_ALV_FIELDCATALOG_MERGE'
        EXPORTING  i_structure_name       = '/CADAXO/SQLCLOGALV'
        CHANGING   ct_fieldcat            = lt_fieldcat
        EXCEPTIONS inconsistent_interface = 1
                   program_error          = 2
                   OTHERS                 = 3.
      IF sy-subrc = 0.
        LOOP AT lt_fieldcat ASSIGNING <l_fieldcat>.
          ...
          APPEND CORRESPONDING #( <l_fieldcat> MAPPING scrtext_m = seltext_m
                                                       scrtext_l = seltext_l
                                                       scrtext_s = seltext_s )
                 TO select_log_fildcats REFERENCE INTO DATA(select_log_fildcat).

          CASE <l_fieldcat>-fieldname.
            WHEN 'SQL_STRING'.
              select_log_fildcat->dragdropid = dragdrop_handle_log.
            WHEN 'UNAME'.
              select_log_fildcat->no_out = 'X'.
            WHEN 'EXECUTE_DATE'.
              select_log_fildcat->outputlen = 12.
              select_log_fildcat->key       = 'X'.
            WHEN 'EXECUTE_TIME'.
              select_log_fildcat->outputlen = 12.
              select_log_fildcat->key       = 'X'.
            WHEN 'RESULT_STATUS_ICON'.
              select_log_fildcat->icon = 'X'.
            WHEN 'SQL_MODE_ICON'.
              select_log_fildcat->outputlen = 12.
              select_log_fildcat->icon      = 'X'.
              select_log_fildcat->just      = 'C'.
          ENDCASE.
        ENDLOOP.
      ELSE.
        MESSAGE e100(/cadaxo/sqlc).
    ...
    <l_cont_grid_result>-gui_alv_grid->set_table_for_first_display(
      EXPORTING  i_bypassing_buffer   = abap_true
                 is_layout            = layout
                 it_toolbar_excluding = g_history_toolbar_excluding
      CHANGING   it_outtab            = gt_history_log
                 it_fieldcatalog      = select_log_fildcats
      EXCEPTIONS OTHERS               = 1 ).
    IF sy-subrc <> 0.
      MESSAGE e100(/cadaxo/sqlc).
    ENDIF.
  ...
  METHOD show_result.
    ...
    DATA l_tabix                    TYPE i.
    DATA l_num2                     TYPE n LENGTH 2.
    DATA l_rows                     TYPE i.
    DATA l_cols                     TYPE i.
    DATA lv_alv_rows                TYPE i.
    DATA l_act_col                  TYPE i.
    DATA l_act_row                  TYPE i.
    DATA lr_cont                    TYPE REF TO cl_gui_container.
    DATA lr_cont1                   TYPE REF TO cl_gui_container.
    DATA lv_footer                  TYPE string.
    ...
    " field symbols
    FIELD-SYMBOLS <l_cont_grid_result> TYPE /cadaxo/sqlcclguicontainer.
    FIELD-SYMBOLS <lr_dref_result>     TYPE REF TO data.
    FIELD-SYMBOLS <lr_sql>             TYPE REF TO /cadaxo/cl_sqlc_cockpit_parse.
    FIELD-SYMBOLS <lt_result>          TYPE STANDARD TABLE.
    FIELD-SYMBOLS <l_result_details>   TYPE /cadaxo/sqlcresult_details.
    ...
    ELSE.
      ...
      " set rows & columns
      calc_result_rows_and_cols( EXPORTING i_lines = l_lines
      ...
      IF g_is_its IS INITIAL.
        gs_splitter_results->set_grid( rows    = l_rows
                                       columns = l_cols ).
      ENDIF.
      ...
      LOOP AT dref_result_tab_t ASSIGNING <lr_dref_result>.
        ...
        ASSIGN gt_cl_sql_parse[ l_tabix ] TO <lr_sql>.
        ...
        <l_cont_grid_result>-gui_container = gs_splitter_results->get_container( row    = l_act_row
                                                                                 column = l_act_col ).
        ...
        <l_cont_grid_result>-gui_splitter = NEW #( parent  = <l_cont_grid_result>-gui_container
                                                   rows    = lv_alv_rows
                                                   columns = 1 ).

        <l_cont_grid_result>-gui_splitter->set_name( l_cont_name ).
        <l_cont_grid_result>-gui_splitter->set_border( border = gfw_false ).
        ...
        <l_cont_grid_result>-gui_splitter->set_row_height( id     = 1
                                                           height = toolbar_row_height ).
        <l_cont_grid_result>-gui_splitter->set_row_sash( id    = 1
                                                         type  = 1
                                                         value = cl_gui_splitter_container=>false  ).
        <l_cont_grid_result>-gui_splitter->set_row_height( id     = 3
                                                           height = toolbar_row_height ).
        <l_cont_grid_result>-gui_splitter->set_row_sash( id    = 3
                                                         type  = 1
                                                         value = cl_gui_splitter_container=>false  ).

        lr_cont = <l_cont_grid_result>-gui_splitter->get_container( row    = 2
                                                                    column = 1 ).
        ...
        <l_cont_grid_result>-gui_alv_grid = NEW #( i_lifetime = cl_gui_alv_grid=>lifetime_default
                                                   i_parent   = lr_cont ).
        ...
        IF gt_lvc_s_layo IS NOT INITIAL.                      "#4170
          ...
          IF l_result_layout IS INITIAL.                      " RT239
            l_result_layout = me->g_result_layout.      " RT239
          ENDIF.                                              " RT239
        ELSE.
          l_result_layout = me->g_result_layout.        " RT239
        ENDIF.                                                "#4170  "RT239

        ASSIGN gt_result_details[ l_tabix ] TO <l_result_details>.
        IF sy-subrc = 0.

          l_result_layout-grid_title = /cadaxo/cl_sqlc_cockpit_main=>build_result_grid_title(
                                           i_runtime = <l_result_details>-runtime
                                           i_lines   = <l_result_details>-lines ).
          " create footer line
          ...
          IF <l_result_details>-restricted_lines IS NOT INITIAL. " CDX130-017
        ...
        SET HANDLER me->on_handle_result_context_menu FOR <l_cont_grid_result>-gui_alv_grid.
        SET HANDLER me->on_handle_result_end_of_page  FOR <l_cont_grid_result>-gui_alv_grid. " CDX Update Add On
        SET HANDLER me->on_handle_result_menu_button  FOR <l_cont_grid_result>-gui_alv_grid.

        lr_cont = <l_cont_grid_result>-gui_splitter->get_container( row    = 1
                                                                    column = 1 ).
        " build and set name of the grid
        ...
        IF <lr_sql> IS ASSIGNED.
          create_dyn_document( EXPORTING i_parent      = lr_cont
                                         i_sql         = <lr_sql>->sql_syntax
                                         i_header_text = <l_result_details>-header_line_text " CR22-034
                               CHANGING  ic_document   = <l_cont_grid_result>-cl_document_header ).
        ENDIF.
        ...
        <l_cont_grid_result>-gui_alv_grid->set_table_for_first_display(
          EXPORTING  i_bypassing_buffer   = abap_true
                     ...
                     it_toolbar_excluding = g_result_toolbar_excluding
          CHANGING   it_fieldcatalog      = <lr_sql>->gt_lvc_t_fcat
                     it_outtab            = <lt_result>
                     it_sort              = lt_lvc_t_sort
                     it_filter            = lt_lvc_t_filt
          EXCEPTIONS OTHERS               = 1 ).
        ...
        IF lv_alv_rows = 3.
          lr_cont1 = <l_cont_grid_result>-gui_splitter->get_container( row    = 3
                                                                       column = 1 ).
          " build and set name of the grid
          CONCATENATE 'CONTAINER_SPLIT_3_' l_num2 INTO l_cont_name.
          lr_cont1->set_name( l_cont_name ).

          create_dyn_document( EXPORTING i_parent    = lr_cont1
                                         i_sql       = lv_footer
                               CHANGING  ic_document = <l_cont_grid_result>-cl_document_footer ).
        ENDIF.
    ...
    " CDX130-017 Begin
    IF l_show_message_restr_lines IS NOT INITIAL.
  ...
  METHOD show_result_tab.
    ...
    ASSIGN dref_result_tab_t[ me->g_active_list_tab ] TO <lr_dref_result>.
    IF sy-subrc = 0.
      show_result_table( i_result_dref = <lr_dref_result>
                         i_tabix       = sy-tabix ).
    ENDIF.
  ENDMETHOD.
  ...
  METHOD show_result_table.
    ...
    DATA lr_cont                    TYPE REF TO cl_gui_container.
    DATA lr_cont1                   TYPE REF TO cl_gui_container.
    DATA lr_tabbar_cont             TYPE REF TO cl_gui_container.
    ...
    <l_cont_grid_result>-gui_splitter->set_row_height( id     = lv_select_id
                                                       height = toolbar_row_height ).
    <l_cont_grid_result>-gui_splitter->set_row_sash( id    = lv_select_id
                                                     type  = 1
                                                     value = cl_gui_splitter_container=>false  ).

    <l_cont_grid_result>-gui_splitter->set_row_height( id     = lv_footer_id
                                                       height = toolbar_row_height ).
    <l_cont_grid_result>-gui_splitter->set_row_sash( id    = lv_footer_id
                                                     type  = 1
                                                     value = cl_gui_splitter_container=>false  ).
    ...
    IF ms_user_settings_xml-reswindoworientation = cs_windowresolution-tab.
      ...
      lr_tabbar_cont = <l_cont_grid_result>-gui_splitter->get_container( row    = 1
                                                                         column = 1 ).
      ...
      ELSE.
        ...
        LOOP AT gt_cl_sql_parse ASSIGNING FIELD-SYMBOL(<lr_sql_button>).
          ...
          ASSIGN gt_result_details[ sy-tabix ] TO <l_result_details>.
        ...
        gr_results_tab_toolbar->add_button_group( data_table = lt_buttons ).
        ...
        gr_results_tab_toolbar->set_registered_events( events = l_events ).
    ...
    lr_cont = <l_cont_grid_result>-gui_splitter->get_container( row    = lv_result_id
                                                                column = 1 ).
    ...
    SET HANDLER me->on_handle_result_user_command FOR <l_cont_grid_result>-gui_alv_grid.
    SET HANDLER me->on_handle_result_context_menu FOR <l_cont_grid_result>-gui_alv_grid.
    SET HANDLER me->on_handle_result_end_of_page FOR <l_cont_grid_result>-gui_alv_grid.
    SET HANDLER me->on_handle_result_menu_button FOR <l_cont_grid_result>-gui_alv_grid.

    lr_cont = <l_cont_grid_result>-gui_splitter->get_container( row    = lv_select_id
                                                                column = 1 ).
    " build and set name of the grid
    CONCATENATE 'CONTAINER_SPLIT_1_' l_num2 INTO l_cont_name.
    lr_cont->set_name( l_cont_name ).
    ...
    IF <lr_sql> IS ASSIGNED.
      IF <lr_sql>->gt_lvc_t_fcat IS INITIAL.
        <lr_sql>->gt_lvc_t_fcat = <lr_sql>->create_alv_field_catalog( i_user_settings   = g_user_settings
                                                                      i_dragdrop_handle = dragdrop_handle ).
      ENDIF.
    ENDIF.
    " COCKPIT-274 END
    ...
    IF lv_alv_rows = lv_footer_id.
      lr_cont1 = <l_cont_grid_result>-gui_splitter->get_container( row    = lv_footer_id
                                                                   column = 1 ).
      " build and set name of the grid
      CONCATENATE 'CONTAINER_SPLIT_3_' l_num2 INTO l_cont_name.
      lr_cont1->set_name( l_cont_name ).
  ...
  METHOD show_saved_lists.
    ...
    /cadaxo/cl_sqlc_cockpit_lists=>get_saved_lists( EXPORTING i_uname         = sy-uname
                                                    IMPORTING e_saved_lists   = gt_saved_lists
                                                              e_free_space_kb = g_free_space_kb ).
    ...
    SET HANDLER me->on_handle_savedlists_usrcommnd FOR <l_cont_grid_result>-gui_alv_grid.
    SET HANDLER me->on_saved_list_select_line      FOR <l_cont_grid_result>-gui_alv_grid.
    SET HANDLER me->on_saved_list_menu_click       FOR <l_cont_grid_result>-gui_alv_grid. " Cockpit-420
    <l_cont_grid_result>-gui_alv_grid->set_table_for_first_display(
      EXPORTING  i_bypassing_buffer            = 'X'
                 is_layout                     = l_lvc_s_layo
                 it_toolbar_excluding          = g_jobmonitor_toolbar_ex
      CHANGING   it_outtab                     = gt_saved_lists
  ...
  METHOD sql_search.
    ...
    CALL FUNCTION '/CADAXO/SQLC_SQL_SEARCH'
      CHANGING cv_search_string = search_string.
  ...
  METHOD store_sql_to_hist.
    ...
    DATA lt_code        TYPE /cadaxo/sqlccodeline_t.
    DATA l_sqlchistline TYPE /cadaxo/sqlchistline.
    ...
    " get the sql text
    IF i_codelines_t IS NOT INITIAL.
      lt_code[] = i_codelines_t[].
    ELSE.
      lt_code   = get_sql_area_lt_code( ).
    ENDIF.
    ...
    IF lt_code[] IS NOT INITIAL.
      ...
      l_cnt_lines = LINES( gt_sql_hist ).

      l_sqlchistline-nr      = l_cnt_lines + 1.
      l_sqlchistline-lines[] = lt_code[].

      IF l_cnt_lines >= 1.
        ASSIGN gt_sql_hist[ l_cnt_lines ] TO <l_sqlchistline>.
  ...
  METHOD tippsandtricks.
    ...
    IF me->g_user_settings-release_type = space.
      g_user_settings-release_type = /cadaxo/cl_sqlc_cockpit_main=>c_release_type_always.
      set_user_settings( me->g_user_settings ).
    ENDIF.

    IF     me->g_user_settings-release_type   <> /cadaxo/cl_sqlc_cockpit_main=>c_release_type_always
       AND me->g_user_settings-release_number  = lv_version.
      RETURN.
    ENDIF.

    CALL FUNCTION '/CADAXO/SQLCTIPPSANDTRICKS'
      IMPORTING  ev_release_type = lv_release_type
      EXCEPTIONS cancel_by_user  = 1
                 OTHERS          = 2.

    IF sy-subrc = 0.
      IF lv_release_type IS NOT INITIAL.
        g_user_settings-release_type = lv_release_type.
      ENDIF.
      g_user_settings-release_number = lv_version.
      set_user_settings( me->g_user_settings ).
    ENDIF.
  ENDMETHOD.
  ...
  METHOD trigger_html.
    ...
    ASSIGN gt_main_classes[ nr = i_main_ref_id ] TO <lwa_main>.
  ...
  METHOD update_field_catalog_alv.
    ...
    LOOP AT gt_cl_sql_parse ASSIGNING <l_cl_sql_parse>.
      ...
      IF <l_cl_sql_parse>->g_saved_list IS INITIAL.
        lt_lvc_t_fcat = <l_cl_sql_parse>->create_alv_field_catalog( i_user_settings   = g_user_settings
                                                                    i_dragdrop_handle = dragdrop_handle ).
      ELSE.

        " read container/grid control
        ASSIGN gcont_grid_result_t[ l_tabix ] TO <l_cl_gui_container>.
        IF sy-subrc = 0.
          <l_cl_gui_container>-gui_alv_grid->get_frontend_fieldcatalog( IMPORTING et_fieldcatalog = lt_lvc_t_fcat ).
          <l_cl_sql_parse>->update_alv_field_catalog_sl( EXPORTING i_user_settings   = g_user_settings
                                                                   i_dragdrop_handle = dragdrop_handle
      ...
      IF me->mv_toolbar_result_active = c_cmd_show_result_table.
        ...
        <l_cl_gui_container>-gui_alv_grid->refresh_table_display( is_stable      = l_lvc_s_stbl                                   " CDX001-0016
  ...
  METHOD update_variant.
    ...
    IF l_string IS NOT INITIAL.
      ...
      gs_sel_variant-t_sql = get_sql_area_lt_code( ).

      symbols_controller->get_user_symbol_from_sql( EXPORTING i_sql     = gs_sel_variant-t_sql
                                                              i_type    = /cadaxo/cl_sqlc_symbols=>cs_symbol_type-user
                                                    IMPORTING e_symbols = lt_symbols ).
      gs_sel_variant-t_symbol = CORRESPONDING #( lt_symbols ).
*      LOOP AT lt_symbols ASSIGNING FIELD-SYMBOL(<ls_symbol>).
*
...
*      ENDLOOP.
  ...
  METHOD usr_action_clear_sql_area.
    ...
    set_sql_area( i_codelines_t = lt_table  ).

    CLEAR gt_errors.

    IF gc_splitter IS NOT INITIAL.

      IF gs_splitter_editor IS NOT INITIAL.
        gs_splitter_editor->set_row_height( id     = 2
                                            height = 0 ).
      ENDIF.

      IF gc_abap_error IS NOT INITIAL.

        gs_splitter_editor->get_row_height( EXPORTING  id                = 2
                                            IMPORTING  result            = g_height
                                            EXCEPTIONS cntl_error        = 1
                                                       cntl_system_error = 2
                                                       OTHERS            = 3 ).
  ...
  METHOD usr_action_pretty_printer.
    ...
    CALL FUNCTION 'PRETTY_PRINTER'
      ...
      TABLES     ntext              = lt_code
                 otext              = lt_code
      EXCEPTIONS enqueue_table_full = 1
                 include_enqueued   = 2
                 include_readerror  = 3
  ...
  METHOD usr_action_show_abap_docu.
    ...
    CALL FUNCTION 'ABAP_DOCU_SHOW'
      EXPORTING area           = 'ABAP'
                ...
                search_mode    = 'E'
                langu          = sy-langu
      IMPORTING docu_container = lcl_gui_control.
  ...
  METHOD usr_action_sql_trace_onoff.
    ...
    IF me->g_sql_trace_on = abap_true.
      ...
      gc_splitter_top_toolbar->set_button_info( fcode = 'TRACETOGGL'
                                                icon  = l_icon ).
    ELSE.
      l_icon = icon_dummy.
      gc_splitter_top_toolbar->set_button_info( fcode = 'TRACETOGGL'
